...
- 갭2 검증 후보: 동결 evidence 의 `l0a/dom.html` — DOM 파일 기준 **E001 mart 참조 56 + E001 격리 4 + E000 9(타깃 6, 중복발사 3) = 69**; 디렉터리 기준 E001 66(dom 없는 6 포함). 리플레이 모집단은 **E001 mart 참조 56** 만(B 16:45 권고, C 동의) — mart 행 조인 가능, 격리·E000 은 보조집합으로 분리 을 오프라인 리플레이 코퍼스로 써 area 신호 검출을 실 DOM 으로 검증 — 네트워크 0, 원본 읽기만. 한계: L1 step 캡처가 없어 endpoint 검출은 오프라인 검증 불가.
