...
| unoccluded_but_offscreen (r4) | F3 | POSITIVE | SELECT_FUNCTION > ENDPOINT_REACHED | = task | 1 | 1 | 0/0 | NONE | 0/F | **null at S0 (GAP-04, control at y≈928 below the fold), 0.0 at S1, s0_task_control_visible = false** | REACHED | S1 |
...
12. **GAP-05 companion (r4, A `T-A-V3-STEP1-012` second example "0 % occluded but off-viewport → not visible")**: `unoccluded_but_offscreen` — the F3 entry control (bbox 16,928 358×90 at S0) has no overlay and nothing ever covers it, but it lies below the 844 px fold and no scroll is performed at S0 → `s0_task_control_visible = false`, `first_visible_scroll_state = S1` (control at y≈342 after the scroll step, centre hit-testable, step occlusion 0.00). **Convention resolved**: the literal "occlusion 0.0 at S0" of the GAP-05 example collides with the GAP-04 null convention (§9: unobserved numerics are `null`, never `0`) — GAP-04 wins for the S0 field, so `task_control_occlusion_s0 = null` and the 0.0 lives in `task_control_occlusion_all_steps` / the walker's step occlusion at the observed state (S1); a runner that reports `0.0` at S0 fails the ±0.02 check exactly as one that reports `visible = true`. Trap: the S0 hero holds the plain text '배송조회…' in a `<p data-c-decoy="TASK_LABEL_TEXT">` so a text-binding runner reports visible=true and fails. No walker change: `inViewport` / `hitCentre` / `occl` already return false / false / null for an off-viewport control (fixture 8 exercises the same path).
...
## walk_fixture.py result (r4 run 2026-08-28 after 3cd6ce9: +1 fixture unoccluded_but_offscreen, walker unchanged since r3; chromium headless 390×844, non-file requests aborted = 0)
...
RESULT: ALL PASS (12/12 fixtures, 1/1 conditional pairs) -> out/walk_result.json
...
r4 mutation control (scratch copy of the lane, `unoccluded_but_offscreen` only, three independent plants + all three at once): claimed `s0_task_control_visible=true` (+ `s0_path_entry_control.visible=true`) → FAIL `s0_task_control_visible observed False != True; entry control in-viewport mismatch`; claimed `task_control_occlusion_s0=0.0` (runner emitting 0 for an unobserved numeric, GAP-04) → FAIL `S0 occlusion(hit-test) None != 0.0; S0 occlusion(geometric) None != 0.0`; claimed `first_visible_scroll_state=S0` → FAIL `first_visible_scroll_state S1 != S0`; all three → one FAIL row listing all five messages. Each run: 11/12 fixtures, 1/1 pairs, walker exit 1; the 11 untouched fixtures stayed PASS.
