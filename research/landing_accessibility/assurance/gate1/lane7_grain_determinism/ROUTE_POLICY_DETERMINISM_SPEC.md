...
| RP-04 | Shortest-path definition | what is minimised (`activation_depth` per `04` §5 / Δ9 token set), **breadth-first enumeration over activation count (MIN-2: length n exhausted before n+1, greedy descent forbidden)**, branch set = Δ30-branch (Δ9 IN 10 + CONDITIONAL 3; SUBMIT_QUERY *is* a branch target), and what happens on equal length (→ RP-03); the report may only claim "minimum observed under BRANCHING_LIMIT · candidate rule · stop rule" (MIN-5, Δ30-claim) |
