#!/usr/bin/env python3
"""control_failure_demo_c.py — C's demonstrator that two C tools honour their DECLARED failure behaviour (Δ46: R40 / Δ46-declared /
...
Demonstrator's own controls (`binding`): in a scratch copy of the assurance layout the demo is run, both tools must report
...
def demo(root: pathlib.Path, repo: str, sidecar: pathlib.Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="cfd_") as d:
        cases = scanner_cases(root, repo, pathlib.Path(d)) + r32_cases(root)
    doc = {
        ...
        "method": "each tool copied (with its fixtures) into an isolated temp dir; one mutation per case; run as a subprocess; r32 measured by the sha256 "
                  "of a pre-seeded --out sentinel before/after (R40: exit codes do not survive in files); scanner measured by its stdout JSON block. "
                  "Live tools are never modified. The sidecar records the LIVE tool sha256 at demo time; the tools compare it to their own sha at run time.",
        ...
        "tool_sha256_at_demo": {SCANNER_REL: sha256_file(root / SCANNER_REL), R32_REL: sha256_file(root / R32_REL)},
        "cases": cases,
        "all_cases_pass": all(c["result"] == "PASS" for c in cases),
        "limitation": "mutations are the ones C imagined (one control-failure path per tool + one crash + r32 exit 3); a PASS here says the declared "
                      "behaviour held under these mutations, not under every possible defect",
    }
...
def binding_control(root: pathlib.Path, repo: str) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="cfd_binding_") as d:
        td = pathlib.Path(d)
        S = td / "assurance"
        (S / "gate1" / "intake").mkdir(parents=True)
        shutil.copy2(root / SCANNER_REL, S / SCANNER_REL)
        shutil.copy2(root / R32_REL, S / R32_REL)
        shutil.copytree(root / FIXTURES_REL, S / FIXTURES_REL, ignore=shutil.ignore_patterns("__pycache__"))
        index_file = td / "V3_RULING_INDEX.json"
        ...
        for rel in (SCANNER_REL, R32_REL):
...
def main(argv: list[str] | None = None) -> int:
    ...
    for c in doc["cases"]:
        o = c["observed"]
        extra = f"changed={o['output_changed']}" if "output_changed" in o else f"status={o.get('ruling_record_gaps.status')} main_keys={len(o.get('main_check_keys_present') or [])}"
        print(f"  {c['result']:11s} {c['case_name']:55s} exit={o['exit']} {extra}", file=sys.stderr)
    bc = None
    ...
    print(f"control_failure_demo_c: {'PASS' if ok else 'FAIL'} cases={sum(c['result'] == 'PASS' for c in doc['cases'])}/{len(doc['cases'])} "
          ...
          f"scanner_sha={doc['tool_sha256_at_demo'][SCANNER_REL][:12]} r32_sha={doc['tool_sha256_at_demo'][R32_REL][:12]}", file=sys.stderr)
