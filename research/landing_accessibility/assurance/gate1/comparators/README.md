...
and `--dry-run` is unchanged (PASS 15 / NOT_TESTABLE 17 at authoring).

## selftest output (2026-08-28, `python3 selftest.py`)
```
...
lane4  clean=PASS (51 items, unmapped=0) | mutated=FAIL fails=3 e.g. ['S1:exactly_once', 'finance_login_gate:never_activate', 'finance_login_gate:cross_fixture_invariants'] | expected-fail checks hit 3/3 → OK
no-map  status=NOT_TESTABLE PASS=0 UNMAPPED=122 → OK
SELFTEST OK: clean synthetic output PASS on 4 lanes, mutated copies FAIL on the predicted checks, no-map ⇒ UNMAPPED (workdir /tmp/claude-1000/-home-sieg-projects-wsl-ProjectFinal/9025a829-6001-41cc-967e-a7eebf607234/scratchpad/gate1_selftest)
```
