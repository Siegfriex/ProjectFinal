...
def main(argv: list[str] | None = None) -> int:
    ...
    if st == "NOT_TESTABLE":  # Δ46-exit2: no check item ran (UNMAPPED / missing runner dirs) — not "ran and failed"
        print(f"compare_lane3: did not run — read neither as pass nor fail (exit 2): {res['summary']['reason']}", file=sys.stderr)
        return 2
...
if __name__ == "__main__":
    try:
        ...
        import traceback
        traceback.print_exc()
        print("compare_lane3: did not run — read neither as pass nor fail (exit 2)", file=sys.stderr)
        _rc = 2
    sys.exit(_rc)
