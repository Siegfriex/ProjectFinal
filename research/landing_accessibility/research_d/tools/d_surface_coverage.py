...
ACCEPTED_UNSURFACED = {
    ...
    ("d_axis_reconcile", "n_columns_in_sidecar"):
        "사이드카 `per_column` 의 열 수(27)다. 스캔은 **대조한 축 11 · 정의 없음 16** 을 찍는데 "
        "그 둘의 합이 이 수다 — **분모가 이미 표시된다**",
    ("d_warn_baseline", "delta"):
        "총수의 증감이다. 스캔이 `총수 399(Δ3 — **신호 아님**)` 로 함께 찍는다 — 이름만 다르다",
    ("d_prov_reconcile", "sidecar_per_column"):
        "사이드카 원문에서 읽은 그 열의 블록이다. 스캔은 `사이드카 27` 로 **그 안의 수**를 찍고 "
        "산문 블록과 어긋나면 **두 수를 나란히** 찍는다",
    ("d_ssot_manifest", "n_missing"):
        "스캔이 `일치 20/20 · 불일치 0 · **없음 0**` 으로 세 수를 다 찍는다 — 이름만 다르다",
