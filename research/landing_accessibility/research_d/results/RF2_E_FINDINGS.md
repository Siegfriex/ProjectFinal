...
1. 
2. prior_archetype 은 gold 가 아니다. 모든 '불일치'는 rule 오류일 수도 prior 오류일 수도 있고 D 는 label 을 열지 않아 가를 수 없다. 2
3. 유형 판정에 쓰인 어휘 사전·경로 마커·구성요소 개수 임계는 D 가 정한 조작화이며 SSOT 에 명문화된 것이 아니다. 3
4. n=56, 유형별 N 이 3~20 이라 유형별 비율의 CI 가 매우 넓다. 4
5. 해결가능성 판정은 반사실(counterfactual
6. 주장이므로 재수집 실험 없이는 검증되지 않는다. 5
7. 곡선은 prior 기준이므로 운영 threshold 결정에 그대로 쓸 수 없다.
