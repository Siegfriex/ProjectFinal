...
생성 `2026-08-27T22:10:32.220009+09:00` · 재실행 `.venv/bin/python research_d/tools/d_dashboard.py`
...
| 섹션 | 한 줄 |
|---|---|
| 1. 운영 상태 | phase=I1 · active MLflow runs A/B/C/D = 0/0/0/1 · open P0 11 · open P1 6 · unpushed agents 2/4 |
...
| 5. Depth | NED/IED/MPFED 전부 0 / 31 task rows — 세 depth 지표 모두 NOT_AVAILABLE. endpoint_reached 31 / 31 task rows. UNRESOLVED 18 행은 종료 사유 미기록. |
...
| 7. D Research | OPEN RQ 9 / 15 queue rows · D runs running 1 · supported 4 / refuted 3 / inconclusive 0 (settled D verdicts 8) |
| 8. C Assurance | C MLflow runs 14 — MATCH 5 / MATCH_WITH_NONBLOCKING_DIFFERENCE 5 / result-affecting+systemic mismatch 4. C completions 12 (severity C1 1 · C0 없음). reproduced D findings: NONE_YET |
...
**phase=I1 · active MLflow runs A/B/C/D = 0/0/0/1 · open P0 11 · open P1 6 · unpushed agents 2/4**

### active runs (T5)
| plane | RUNNING runs | 분모 |
|---|---|---|
| A | 0 | 47 total MLflow runs |
| B | 0 | 47 total MLflow runs |
| C | 0 | 47 total MLflow runs |
| D | 1 | 47 total MLflow runs |
...
|---|---|---|---|---|
| A | I1 | ALL_DECISIONS_ISSUED_AWAITING_W1_W4_COMPLETIONS | 996 | 8 |
| B | I1 | I1_P0_CONTAMINATION_SELF_REPORTED | 11 | 0 |
| C | I1 | HOLDOUT_EXPOSURE_CORRECTED_TO_26__BLIND_PILOT_HOLDOU | 128 | 2 |
| D | I1 | RESEARCHING | 175 | 0 |

current phase = **I1**

### open P0/P1 — P0 **11** · P1 **6** (분모 41 P0+P1 tickets) (T4)
...
| D-ATTESTATION-001 | P0 | FACT_CORRECTION | D→A | A |  |
| C-FINDING-221007 | P0 | FINDING | C→A,B | A,B |  |
| T-B-W2-ATTEST-001 | P0 | FACT_CORRECTION | B→A,C | A |  |
| T-A-DIRECTOR-A1-A7 | P0 | DIRECTIVE | A→B,C,D | B,C,D |  |
...
|---|---|---|
| A | `control/landing-orchestrator` | `4298172da12c80759deb9a9d4ea231197de3893f` |
| B | `claude-b/clean0-v21` | `95650b2f574a02c4c4e988786f0485924d3e8cab` |
...
| C | `claude-c/assurance-v21` | `8b2d6ee481e3b69eab7113c725d2b55c2afa9d7c` |
| C | `claude-c/assurance-current` | `1baa865b4a673af05033e6e6289fd2713676baa5` |
| D | `claude-d/research-sandbox-v21` | `2ad18b69859e79a87d48fef94cee3366f9ea39ef` |
...
|---|---|---|---|---|
| A | `control/landing-orchestrator` | `9f39acf20b8b` | `4298172da12c` | **X** |
| B | `claude-b/clean0-v21` | `95650b2f574a` | `95650b2f574a` | O |
| C | `claude-c/assurance-v21` | `b7d13fbe20c4` | `8b2d6ee481e3` | **X** |
| D | `claude-d/research-sandbox-v21` | `2ad18b69859e` | `2ad18b69859e` | O |
...
criterion result rows = 0 criterion result rows in frozen mart · dim_certification rows = 0 certification dimension rows

## 5. Depth (Axis B)

**NED/IED/MPFED 전부 0 / 31 task rows — 세 depth 지표 모두 NOT_AVAILABLE. endpoint_reached 31 / 31 task rows. UNRESOLVED 18 행은 종료 사유 미기록.**
...
| endpoint reach | 31 / 31 task rows | T3 |  |
...
auth gate before endpoint: 31 / 31 task rows
...
| semantic classified rate | 0.5319 / 235 interrupt element rows | T3 | classification_status=DETERMINISTIC |
| UNKNOWN label share | 0.4681 / 235 interrupt element rows | T3 | final_label=UNKNOWN |
...
| blocks_primary_action | 235 / 235 interrupt element rows | T3 | page-level |
| dismiss control exists | 235 / 235 interrupt element rows | T3 |  |
...
**OPEN RQ 9 / 15 queue rows · D runs running 1 · supported 4 / refuted 3 / inconclusive 0 (settled D verdicts 8)**

| 항목 | 값 | tier |
|---|---|---|
| open research questions | 9 / 15 RQs in queue | T5 |
| experiments running | 1 / 11 D-plane runs | T5 |
| supported (+partially) | 4 / 8 D runs with a settled verdict | T5 |
| refuted (+not supported) | 3 / 8 D runs with a settled verdict | T5 |
| inconclusive (+not testable) | 0 / 8 D runs with a settled verdict | T5 |

> verdict 는 D 자신이 붙인 tag 다(T5). C 검증을 통과했다는 뜻이 아니다. D run 의 authority_status 는 전부 NON_CANONICAL 이다.

**latest high-impact finding** — `RQ-D-RF-001` verdict **PARTIALLY_SUPPORTED** (run `bd04b82fa110`, 2026-08-27T22:10:30.277297+09:00)

> limitation: n=56 에 7 class, 5개 class 가 n<=5 라 per-class 추정의 Wilson CI 가 거의 [0,1] 폭이다. target 은 gold label 이 아니라 business-domain prior 이므로 불일치가 모델 오류인지 prior 오류인지 이 실험은 구분할 수 없다. 입력 DOM 품질 결함(중복 blob 2건, 인코딩 깨짐 의심 7건, 토큰 12 이하 사실상 URL only 6건)이 상한을 누른다. MiniLM 은 max_seq_length=128 이라 긴 blob 이 잘린다.
...
**C MLflow runs 14 — MATCH 5 / MATCH_WITH_NONBLOCKING_DIFFERENCE 5 / result-affecting+systemic mismatch 4. C completions 12 (severity C1 1 · C0 없음). reproduced D findings: NONE_YET**

| 항목 | 값 | tier |
|---|---|---|
| MATCH | 5 / 14 C-plane runs | T5 |
| result-affecting + systemic mismatch | 4 / 14 C-plane runs | T5 |
| C completions severity C1 | 1 / 12 C completions | T4 |
| C completions severity C0 | **NONE_YET** | T4 |
| **reproduced D findings** | **NONE_YET** | T4 |

> reproduced D findings 가 없는 이유 — bus 전체(acks/completions/C 티켓)에서 C 가 작성한 파일 중 RQ-D 를 언급한 것이 0건이다. D 는 D-RESEARCH_FINDING 을 방금 발행했고 C 는 W1/W2/W4 acceptance 검증 중이다 — 재현 요청이 C 큐에 도달했지만 아직 처리되지 않았다는 뜻이다.

> C 가 D 를 재현하지 않았다는 것은 D 결과가 틀렸다는 뜻도, 맞다는 뜻도 아니다. D run 은 전부 authority_status=NON_CANONICAL 이며 C 검증 없이는 그대로 남는다.
...
이 대시보드는 **NON_CANONICAL** 이다. D plane 산출물이며 GO 권한이 없다. MLflow run: `ab758ce3b7be4134b87cc175784a2e05`
