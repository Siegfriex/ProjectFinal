...
# §17 — 산출물이 사람에게 어떻게 읽히는가 (2026-08-28 15:45)

A 가 `T-A-V3-K8-002` R159 · `T-A-V3-K8-003` R163 에서 이 축을 새로 세웠다.
**지금까지의 검사는 값 검사 vs 산출물 비교였는데, 이건 산출물의 데이터 vs
산출물의 표현이다.** 값이 맞아도 읽을 수 없거나 잘못 읽히면 틀린 것이다.

D 와 B 가 각자 자기 쪽에서 같은 것을 관측했다:

| | 검사 밖에 있던 것 |
|---|---|
| **D** | 그림의 **표현** — "검사는 색을 보지 레이아웃을 안 본다" |
| **B** | 값의 **해석** — "`RETRACTIONS.md` 를 CSV 옆에 뒀는데 그것도 검사가 아니라 문서다" |

## §17.1 그림은 렌더해서 본다 [A R159]

**자동 검사로 대체하지 않는다.** sha · exit 0 · 번들 검사가 셋 다 통과한 그림에서
레이아웃 결함 3건이 나왔다 — 2줄 제목이 상단 텍스트와 겹침 · `x=1.0` 두 점의
라벨 포개짐 · 범례가 데이터점을 가림.

최소한 이 셋은 **사람이 확인한다**: **라벨 겹침 · 범례 가림 · 제목 충돌.**

## §17.2 철회는 도구가 안다 [A R163 / D-DEF-48]

주석은 검사가 아니고 문서도 검사가 아니다. `tools/d_retractions.py` 가 정본
`CANONICAL_MART_50.RETRACTIONS.md` 를 파싱한다.

| 층 | 무엇 |
|---|---|
| `cite()` | 철회 토큰에 `[RETRACTED]` 를 붙여서만 문자열로 내보낸다 |
| `audit_artifacts()` | 산출물(analysis/*.csv·md·json)의 표시 없는 인용 |
| `audit_tickets()` | **발행 티켓**의 표시 없는 인용 |
| `d_emit_ticket.retraction_errors()` | **발행 전 차단** |
| `d_bus_scan.sh` | 매 루프 노출 |

JSON 은 키를 오염시키지 않는다 — `retracted_labels_cited` 로 선언한다.
키에 표시를 붙이면 그 파일을 읽는 도구가 키를 못 찾는다.

**시점 경계가 있다.** 철회 이전 인용은 위반이 아니다. 성립 시각은 정본이 든
근거 티켓 중 **가장 이른 것**이며(근거가 나중에 교체돼도 성립 시점은 처음),
소급해서 세면 6건이 다 위반으로 보인다 — 실제 위반은 1티켓이다.

## §17.3 개명이 사전등록과 부딪힐 때

철회 라벨을 고치면 그 라벨로 고정된 사전등록이 FAIL 로 바뀐다.
**사전등록 원문은 고치지 않는다** — 결과에 맞춰 사전등록을 바꾸면 목적이 없어진다.
**정본이 대체 라벨을 지정한 개명만** 인정하고, 대조군에 둘을 박아 둔다:

- 정본에 없는 **임의 개명은 막힘**
- 개명이 인정돼도 **수치가 다르면 막힘**

여기가 "이름이 달라졌으니 예외를 준다" 가 "수치가 달라졌으니 예외를 준다" 로
미끄러지는 자리다.

## §17.4 A 판정 수용 기록 — `T-A-V3-K8-004` (2026-08-28 15:52)

A 가 R164~R169 로 D 의 시정을 전부 승인했다. 그리고 **D 가 몰랐던 상류 사실**을
밝혔다.

**`SITE-SIDE ROUTE NOT OBSERVED` 는 A 가 `T-A-V3-TBX-022` 에서 쓴 이름이다.**
D 는 그것을 실었다 — 21건 인용의 출처는 A 의 티켓이고, 철회를 발행한 손이
철회된 함의를 계속 재생산한 것이다. **문서에 있고 검사에 없으면 이렇게 된다.**

### 차단이 어디까지 되는가 — 층위를 분명히 한다

A 는 R169 에서 "자동화는 사후 탐지까지이고 사전 차단이 아니다" 라고 했다.
**같은 회차에 티켓 경로는 사전 차단으로 바뀌었다.** 층위를 나눠 적는다:

| 층 | 상태 |
|---|---|
| **발행 티켓** | **사전 차단** — `d_emit_ticket.retraction_errors()` |
| **산출물** | 사후 탐지 — 생성 코드가 `cite()` 를 안 거치면 그 순간엔 안 막힌다 |
| **그림 표현** | 사람이 본다 — 자동화 없음 (§17.1) |

**"자동화는 사후 탐지까지" 는 산출물·그림에는 맞고 티켓 경로에는 더 이상
맞지 않는다.** 남은 두 층을 사전 차단으로 옮기는 것이 다음 자리다.

### R167 — 스키마는 A 가 넓힌다

`ADDENDUM`·`STATUS` 를 enum 에 넣되 **정의를 함께 적는 것**이 A 의 조건이다.
D 검사 `d_ticket_schema_check` 는 **정본 파일을 읽으므로 D 쪽 수정이 필요 없다** —
넓히면 자동 반영된다. 다만 enum 10건이 해소돼도 `missing:status` 49 ·
`missing:scope` 30 은 남는다. **이미 발행된 것이라 고치지 않는다.**
