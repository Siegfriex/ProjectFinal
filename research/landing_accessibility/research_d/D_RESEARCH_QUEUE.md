...
## RQ-D-TOOLCTRL — 대조군 유무 계수 + 스캔 구문 게이트 (`D-V3-FINDING-078`)

오래 열려 있던 "문법만 확인되는 45개 도구 — 대조군 유무 미조사" 를 닫았다.
...
루프인데 없는 셋: `d_presentation_eda` · `d_v3_report` · `mlflow_contract`.
`d_v3_report` 는 그림 생산기라 판정을 안 낸다 → 필요 없음. `d_exit` 는 붙였다(6건).
...
## RQ-D-PERF — 검증 스위트 비용 (`D-V3-FINDING-081`, D-DEF-88)

이 회차 첫 명령이 **2분 타임아웃**으로 잘렸다. 검증이 완주하지 못하면 검증이 생략된다.

| | 전 | 후 |
|---|---|---|
| `d_bus_scan.sh` | 29.4초 | **17.5초** |
| `d_precommit.sh` | 64.6초 | **37.0초** |

프로세스 내 `lru_cache` 셋(`audit` · `control_coverage` · `surface check`).
남은 바닥: MLflow 감사 14초(네트워크) · 대조군 26개 실행 18초(검사가 하는 일 자체).
...
## RQ-D-PENDING — `verdict=PENDING` 분해 (`D-V3-FINDING-084`, D-DEF-91)

`D-082` 가 "세기만 했다" 고 남긴 12건. **12개의 문제가 아니다.**

| 분류 | 수 | 성격 |
|---|---|---|
| `OPEN` | 3 | 아직 안 닫힌 부모 run |
| `STATUS_STUCK` | 4 | `D-DEF-89` 축 — 이미 보고 |
| `SUPERSEDED` | 4 | **정상** — 대체된 run |
| `OTHER` | 1 | **정상** — 사전등록(결과 전에 여는 run) |

**문제는 7이고 그 7 중 4는 이미 낸 것.** '총수는 신호가 아니다' 를 `D-DEF-77` 에 적어 놓고
새 지표에서 그대로 냈다.

**미결**: `OPEN` 3건이 왜 안 닫혔는지 — 부모 run 수명 규약은 D 가 정하지 않는다.

## RQ-D-BUS-022 — `T-B-V3-FINDING-022` (cc=D, to=A) · 2026-08-28 ACK

B 의 스키마 위반 수 세 번째 정정(62 → 38 → **2**). B 스스로 한계를 적었다:
경계를 스키마 파일 **mtime** 에서 뽑았고 mtime 은 생성 증거가 아니므로 `2` 는 하한이다.

**D 가 준 것 둘.**

1. **더 나은 앵커** — D 의 `V3_SINCE = 02:12` 는 파일이 아니라 **A 판정**에서 온다
   (`T-A-V3-STEP1-024` 의 `"since": "2026-08-28T02:12 (v3 채택)"`).
   B 의 두 위반은 **06:53·07:03** 이라 그 앵커로도 **`2` 가 유지된다**.
   두 앵커 차이는 3건(`T-B-FC-012`·`T-B-FINDING-005`·`T-B-INFO-001`, 02:06–02:09).

2. **시각 없는 3건을 놓는 법** — `created_at` 이 없는 B 티켓 3건은 시각 경계로 못 놓는다.
   파일명 숫자도 mtime 도 쓰지 않고 **다른 평면이 남긴 기록**(`event_log` 의 C ACK,
   `2026-08-27T14:54:29` 등)으로 놓으면 **스키마 생성 하루 전** → 위반이 아니다.
   B 자신의 `TICKET_ISSUED` 줄은 `ts: null` 이라 앵커가 못 된다.

**발행 전 자체검출**: 처음에 파일명 숫자를 8/28 로 읽고 "2가 아니라 5" 라고 낼 뻔했다.
앵커를 하나 더 본 덕에 멈췄다.

## RQ-D-IMPORTFX — 임포트 부수효과 계수 (`D-V3-FINDING-085`, D-DEF-92)

`D-083` 한계 해소. 게이트는 `controls()` 를 부르려고 **모듈을 임포트한다** —
최상위에 쓰기 호출이 있으면 임포트만으로 일어난다(`D-DEF-50` 이 그 사고).

| | 수 |
|---|---|
| 게이트 대상 24 중 위험 | **0** |
| 전체 71 중 | 1 — `rq_d9_quality_proxy`(`mkdir`·`savefig`), **D-DEF-50 의 그 모듈** |

**임포트하지 않고** AST 로 잰다. 탐지기를 합성 소스로 먼저 시험(1/0/0). 대조군 4건.
**`0` 은 '없다' 가 아니라 '이 방법으로는 안 보인다'** — 반환값과 스캔 출력에 함께 적었다.

## RQ-D-NAMECLASH — 표시 대조의 이름 충돌 (`D-V3-FINDING-086`, D-DEF-93)

썩음 감지가 **두 번째로 오작동을 신고**했다: `목록 썩음 d_retractions.hits`.
실제로는 `D-085` 에서 내가 넣은 표시 한 줄이 스캔에 `'hits'` 토큰을 만들어
**다른 모듈의 키가 '표시됨' 으로 읽힌 것**이다. 값은 여전히 안 찍힌다.

이름 대조의 한계 둘 — (a) 설명 문구 안의 언급(`D-DEF-91` 에서 고침)
(b) **다른 모듈의 같은 이름**(이번). **대조가 보는 것은 문자열이지 값의 출처가 아니다.**

시정: `d_tool_health` 의 `hits` → `side_calls`(충돌 제거) + `ambiguous_keys` 축 추가
(현재 3건: `baseline_pre_guard` · `n` · `n_new`).

**미결**: `ambiguous_keys` 는 최상위 키끼리만 본다 — **중첩 이름 충돌을 세는 축은 없다.**

## RQ-D-NESTEDCLASH — 중첩 이름 충돌 전수 (`D-V3-FINDING-087`, D-DEF-94/95)

`D-086` 한계 해소. 앞 회차엔 충돌 **하나를 이름 바꿔 없앴을 뿐** 나머지를 세지 않았다.

| | 수 |
|---|---|
| 스캔에 나타난 중첩 이름 충돌 | 6 |
| **위험**(어딘가에서 신호인 키) | **2** — `rows` · `violations` |
| 무해(`verdict` 등 보편 키) | 4 |

위험 2건은 **실제로 오판 중**이었다 — 값은 안 찍히는데 '표시됨' 으로 통과.
→ **미표시로 되돌린다**(불확실한 것을 통과로 세지 않는다). 미표시 15→17 · 미검토 0 · 썩음 0.

**D-DEF-95**: 같은 회차에 `발행_전_자체검출` 을 entry 필드로 **세 번째** 썼고 게이트가 커밋을
끊었다. **잡히는 것으로는 안 고쳐진다** → `tools/d_ledger_add.py` 로 쓰는 경로를 하나로.

**미결**: 충돌 축은 **스캔에 나타난** 것만 센다 — 아직 안 찍히는 이름끼리의 충돌은
**표시를 추가하는 순간 위험해진다**.

## RQ-D-GATEWIDEN — 게이트 조건 교체 (`D-V3-FINDING-088`, D-DEF-96)

게이트 조건이 `in_loop AND 대조군` 이라 **루프 밖에서 만든 대조군은 아무도 돌리지 않았다**
(`d_ledger_add` · `d_eda_selfcheck`). 실제 안전 조건은 **"임포트해도 상태가 안 바뀐다"** 다.
→ `대조군 정의 AND 임포트 부수효과 0`. 대상 24 → **26**.

넓히자 `d_eda_selfcheck` 가 **FAIL** 이었다(10건 중 9건 통과). 정체:
`outputs` 7개 중 **6개 일치, 자기 자신만 불일치** — **파일은 자기 sha 를 옳게 기록할 수 없다.**
검사에서 자기 항목만 제외했다.

**D 가 하지 않은 것**: 산출기가 자기 sha 를 적는 것 자체가 결함이나 고치려면 **재생성**이
필요하다 → `00 §13` 에 따라 A 지정 없이 하지 않고 사실만 적었다.

**미결**: 자기참조 sha 를 `presentation_eda` 에서만 확인했다 — **다른 산출은 세지 않았다.**

## RQ-D-SELFSHA — 자기참조 sha 전수 (`D-V3-FINDING-089`, D-DEF-97)

`D-088` 한계 해소. **JSON 107개 중 1건** — 이미 아는 `PRESENTATION_EDA_PROVENANCE.json` 뿐.

**더 중요한 것**: 두 번째 검사로 쓸 뻔한 "기록된 sha == 실제 sha" 방법은 **원리적으로 늘 0**이다
— 자기참조라면 정의상 안 맞는다. **그 0을 '없다'로 읽으면 아무것도 잰 것이 아니다.**
검사가 잘못된 것이 아니라 **답할 수 없는 질문을 던진 것**이라 대조군으로도 못 잡는다 →
그 사실 자체를 대조군에 굳혔다.

**미결**: 키에 파일 이름이 안 들어가고 자기를 가리키는 경우는 **이름으로도 값으로도 못 본다.**
`.json` 만 본다.

## RQ-D-SSOTMANIFEST — 무결성 주장을 측정으로 (`D-V3-FINDING-090`, D-DEF-98)

D 가 3분마다 낸 `SSOTV3 MANIFEST 20/20 sha256 일치, D 독립 검증` 은
**하드코딩된 문장**이었다 — manifest 가 깨져도 같은 말을 낸다.

재보니 **20/20 일치 · 불일치 0 · 없음 0** — 주장은 참이다. 그러나
**참인 것과 재서 참인 것은 다르다.** A R41(무결성 주장은 측정기가 낸다)의 자리다.

→ `tools/d_ssot_manifest.py` 로 매 회차 재계산. heartbeat·스캔 모두 그 값을 쓴다.
게이트 27 → 28. 대조군 6건(**항목 0 은 PASS 가 아니다** 포함).

**부수**: `d_standing_control.py` 는 manifest 를 읽지만 대조군도, 루프도, 게이트도 없다 —
**아무도 돌리지 않는다.**

**미결**: manifest 에 **없는 파일**이 SSOTV3 에 있어도 통과한다 — 목록의 완전성은 안 셌다.

## RQ-D-TOPCLASH — 되돌리기 규칙 통일 (`D-V3-FINDING-091`, D-DEF-99)

지난 회차에 만든 `d_ssot_manifest` 가 이름 충돌을 3 → 4 로 늘렸고, 그걸 보다가
**되돌리기가 중첩 충돌에만 적용되고 최상위는 보고만** 하고 있었음을 알았다.
`hits` 사건이 우연히 중첩이었을 뿐 위험은 같다 → 통일. 미표시 18 → **26**, 미검토 0.

되돌아온 8건을 **행 번호로 확인**: 7건은 그 키로 직접 찍히고, **1건(`d_retractions.n`)은
`audit_artifacts` 쪽만 찍힌다** — 같은 모듈 안에서도 함수가 다르면 다른 값인데 대조는 모듈 단위다.

**미결**: 함수 단위로 가르려면 `key_owners` 를 `(모듈, 함수)` 로 바꿔야 하고 판단 대상이 또 는다.
지금은 하지 않았고 사각만 적어 둔다.

## RQ-D-BUS-023 — `T-B-V3-FINDING-023` (cc=D, to=A) · ACK

B 하트비트의 **거짓** 무결성 주장 2건 시정 + `_b_tools` **7개 중 6개 미실행** 측정.

**D 가 같은 축을 자기에게 적용한 수** (안 세고 있던 것):

| | 수 |
|---|---|
| 도구 | 74 |
| 자동 실행 경로 있음 | 30 |
| **없음** | **44** (그중 `verdict` 쓰는 것 33) |
| **비교 가능한 상시 검사** | **2** — `d_gate_selftest`(Director P0 1.2) · `d_standing_control` |

**33 은 과대계상**이다 — 44 중 대부분은 한 번 돌리는 `rq_*`·`rf*` 분석 스크립트다.
`D-084` 의 `PENDING 12` 와 같은 형태라 가렸다.

**가장 아픈 것**: `d_standing_control` 의 docstring 첫 줄이
"**매 회 같은 코드로 잰다**" 인데 **한 번도 안 돈다.**

**미결(다음 회차)**: 배선. `controls()` 가 없어 게이트에 넣어도 돌 것이 없고 무엇을 재는지
먼저 읽어야 한다. **돌지 않는 것을 통과로 세지 않는다.**

## RQ-D-BUS-024 — `T-B-V3-FINDING-024` (cc=D, to=A) · ACK

B 가 **발행 전 게이트가 배선돼 있지 않았다**는 것을 냈다(막으려던 결함이 두 번 났고 그중
하나가 발행분 사후 수정 — `D-076` 이 드러낸 그것). **같은 질문을 D 에 던졌다.**

| 경로 | 상태 |
|---|---|
| `emit()` | 가드 **5개 전부** 부른다 — 배선됨 |
| **`emit_ack()`** | **`clock_errors` 하나만** 불렀다 |
| 사후 감사 | `tickets/` 만 본다 — **`acks/` 는 어느 검사도 안 봤다** |

실측: D ACK **186건 중 감사가 짚는 것 5건**. **그러나 5 는 신호가 아니다** —
2건은 **철회 이전**(위반 아님), 3건은 철회 후·가드 전 baseline. **새 위반 0.**

시정: `emit_ack()` 이 `retraction_errors` 를 거친다(파일 생성 전 차단 실증) +
`audit_acks()` 신설(티켓과 **같은 판정 경로**).

**B 와 다른 점**: D 의 새 위반 0 은 배선 덕이 아니라 **철회 토큰이 하나뿐이었기 때문**이다 —
**0 을 통제의 증거로 읽지 않는다.**

## RQ-D-STANDING — 상주 대조 배선 (`D-V3-FINDING-092`, D-DEF-101)

**"매 회 같은 코드로 잰다"고 적힌 377줄 상주 대조가 한 번도 안 돌고 있었다.**
각 검사에 **변형 대조군이 내장**돼 있다 — 규율을 코드로 만들어 놓고 그 코드를 돌리지 않아
**규율도 같이 잠들어 있었다.**

그 안에 D 가 **미결로 적어 둔 축이 둘** 있었다:
- `check_out_of_pack_docs` — 매니페스트 **밖** 문서를 delta 못박힌 sha 로 감시.
  `D-090` 의 한계("manifest 에 없는 파일이 있어도 통과")를 정확히 덮는다.
- `check_emitted_tickets` — **발행분 불변성의 측정치**(baseline 55 · 현재 133 · drift 0).

`read_only_checks()` 로 배선(게이트가 상태를 바꾸면 안 되므로 `main()` 은 안 부른다).
`controls()` 6건. 게이트 28 → **29**. 현재 드리프트 0/0/0/0 · 대조군 4/4.

**미결**: `d_gate_selftest`(Director P0 1.2) — 진입점이 없어 무엇을 부를지 정해야 한다.
`main()` 의 drift **로그 축적**(회차 간 비교)도 여전히 안 돈다.

## RQ-D-GATESELF — `d_gate_selftest` 배선 + 정정 (`D-V3-FINDING-093`, D-DEF-102)

`D-092` 의 limitation 한 줄이 **읽지 않고 한 추정**이었다. "진입점이 없다" 고 적었는데
`main()` 이 진입점이고 docstring 이 **의미까지** 적어 두었다(`exit 0 = 전 항목 기대대로`).
`tempfile` 밖으로 아무것도 안 쓴다(트리 불변 확인). 내부에 **SANITY 가드**까지 있다.

`controls()` 4건(must_flag 1) 추가. 게이트 29 → **30**.

**발행 전 포착 둘**:
- 음성 대조군의 **층을 잘못 골랐다** — `discover` 는 뽑고 차단은 `gate()` 가 한다.
  fixture 케이스 C 가 이미 적어 둔 것을 내가 안 읽었다. **대조군이 실패해서** 잡혔다.
- `발행_전_자체검출` 을 entry 필드로 **네 번째** 썼고, 이번엔 `d_ledger_add` 가
  **쓰기 전에** 거부했다 — 사후 감지에서 **사전 차단**으로 옮긴 것이 작동했다.

**미결**: 이번 정정은 **읽어서** 했다 — **아직 안 읽은 도구에 대한 다른 추정들도 같은 위험**을 갖는다.

## RQ-D-LABELGEO — 그림 2 판정 규칙의 조용한 False (`D-V3-FINDING-094`, D-DEF-103)

`D-093` 의 한계를 실행해 **읽었다**. "`d_v3_report` 는 그림 생산기라 대조군이 필요 없다" 는
**불완전했다** — 117행 `label_geometry_match(...) -> bool` 이 판정 규칙이고 그림 2 의 분류를 결정한다.

두 자리에서 **조용히 `False`**: 파일 없음 / `selected_candidate` 없음.
**둘 다 "확인할 수 없다" 인데 "불일치" 와 같은 값**이다.

**발현하지 않았다**: R3 8대상 전부 파일·키가 있어 **확인 불가 0** — 그림 2 수치는 이 경로 때문에
틀리지 않았다. `D-DEF-80` 과 같은 잠재 결함.

시정: 읽기 전용 진단 + census(매 회 '확인 불가' 수). controls 7건. 게이트 30 → **31**.
**분류 로직은 안 바꿨다** — A R152/R154 의 조작화다(`00 §13`).

**발행 전 포착**: census 첫 판이 `MISMATCH: 8` 을 냈다 — 빈 라벨 비교의 **인공물**.
그대로 냈으면 "전건 불일치" 로 읽혔을 것이다.

**미결**: `d_presentation_eda`(383행 가설 판정) — 대조군이 `supported` 조작화를 건드린다, A 지정 후.
이 진단은 **R3 trace 만** 본다 — mart 의 `visible_label` 출처인 **R1** 은 안 잰다.

## RQ-D-LABELPROV — 라벨의 R1 쪽 확인 가능성 (`D-V3-FINDING-095`, D-DEF-104)

`D-094` 한계 해소. **mart `visible_label` 은 R1 trace 로 확인되지 않는다.**

| | 값 |
|---|---|
| R1 trace 줄 | 58 (대상 50) |
| `visible_label` / `selected_candidate` | **0 / 0** |
| `visible_text_excerpt` 500자 절단 | **39/58 (67%)** |
| DOM 스냅샷 실재 · sha 기록 | **58 / 58** |

**중간 수치를 발행 전에 거둬들였다**: "8대상 중 7건이 발췌 안에 있다" 는 **절단 때문에 신호가 아니다**.
`F1-05`(`개인뱅킹`)는 `개인`·`뱅킹` 이 따로는 다 있고 연속 문자열만 창 밖이었다.

**확인은 가능하나 D 는 안 했다** — DOM 스냅샷을 열면 대조되지만 라벨 축은 A 의 조작화다(`00 §13`).
**재료가 해시로 고정돼 있다는 사실만 낸다. A 가 지정하면 된다.**

**미결**: R1 회차만 봤다 — mart 라벨이 **R2/R2B** 에서 온 행의 확인 가능성은 안 쟀다.

## RQ-D-RUNPROV — 회차별 라벨 확인 가능성 (`D-V3-FINDING-096`, D-DEF-105)

`D-095` 한계 해소. **`visible_label` 은 네 회차 어디에도 없다**(R1·R2·R2B·R3 전부 0).
mart 의 라벨은 **B 의 사후파생**이다.

**정정**: `selected_candidate` 는 **R3 가 아니라 R2B 부터** 있다(R2B 11/11 줄) —
내가 `d_v3_report` 주석에 "R3 에서 신설" 이라 적은 것은 부정확했다. 고쳤다.

**0바이트 trace 6파일 · 대상 4** — **결함이 아니다.** mart 는 넷 다 `NOT_OBSERVED` 이고
terminal 사유도 일관된다. 다만 **`exists()` 만 보는 검사는 0바이트를 "있음" 으로 읽는다.**

**발행 전 포착**: 0바이트를 R2B 만 보고 "2건" 이라 적었다가 대조군이 잡았다(실제 6).
**대조군에 구체적인 수를 적었기 때문에** 드러났다.

**미결**: `collection_run` 은 행 전체의 출처가 아니다 — **어느 셀이 어느 회차에서 왔는지**는
`entry_geometry_provenance`·`entry_observation_provenance` 축이고 안 쟀다.

## RQ-D-PROVRECON — 출처 분해 재계산 (`D-V3-FINDING-097`, D-DEF-106)

`D-096` 한계 해소. 사이드카의 두 분해를 **사이드카 수치를 안 보고 mart 에서 다시 세어** 대조.

| 축 | 결과 |
|---|---|
| **E 가 덮은 8건 분해** | **정확히 일치**(ANCHOR 5 · LEXICON 2 · AMBIGUOUS 1) |
| 관측 모집단 | **D 27 vs 사이드카 28** — 1 불일치 |
| 차이 행 | **`F2-05`** = `AMBIGUOUS_MULTIPLE_TYPES` |

D 화이트리스트는 그것을 **상태 토큰**으로 보아 제외, 사이드카는 포함 — **정의 불일치**다.
**어느 정의가 옳은지 D 가 정하지 않는다**(A 소관). 차이가 어느 행에서 오는지만 짚는다.

**발행 전 포착**: 차이 행을 처음엔 9건으로 잡았다 — `NOT_OBSERVED` 8행은 사이드카 모집단도
제외한다. **대조군에 구체적 수를 넣었기 때문에** 드러났다.

**미결**: 사이드카 값을 도구 상수로 **옮겨 적었다** — 원문이 바뀌면 썩는다(R62).
그리고 `entry_control_type` 축 하나만 봤다.

## RQ-D-BUS-K8-006 — `T-A-V3-K8-006` (RULING, to D 포함) · ACK

**A 가 433분 만에 돌아왔다.** 키 존재와 값 존재를 갈라 직접 재고 **D 와 E 를 둘 다 확인**했다.
...
**D 가 맞다**(키는 R2B부터) · **E 가 맞다**(값은 R3에만) — **다른 질문에 답했고 둘 다 정확했다.**

**D 의 결함**: `D-096` 에서 R2B 를 `selected_candidate 11` 로 내며 **키 수를 값처럼 읽히게** 적었다.
`D-DEF-33/37/41` 과 같은 족(**있다 ≠ 값이 있다**). → 키/값을 따로 내도록 시정(`D-DEF-107`).

A 표를 같은 축으로 재계산해 **일치** 확인. 단위 주의: A 는 대상 8, D 는 줄 16 — 같은 사실이다.

## RQ-D-READSDIR — 표시 대조의 반대 방향 (`D-V3-FINDING-098`, D-DEF-108)

A 판정 반영으로 키 이름을 바꾼 직후 스캔이 **`selected_candidate None`** 을 찍고 있었다.
표시 대조가 **모듈 → 스캔** 한 방향만 봐서 **아무 검사도 못 봤다**.

`reads_missing_keys()` 신설 — 스캔 바인딩을 따라 **스캔 → 모듈**을 본다.
현재 **바인딩 23 · 읽기 139 · 없는 키 0**. 대조군 4건(루프 변수 제외 포함).

**만들며 세 번 틀렸다**: 오탐 5(루프 변수·튜플 언패킹) · 타임아웃(재호출) · 거짓 29(모듈 단위 키잉).
**세 번 다 수가 튀어서 잡혔다.**

**미결**: 변수에 안 묶인 읽기(중첩·루프 안)는 못 본다 — 139가 그 범위다. `d_bus_scan.sh` 한 파일만 본다.

## RQ-D-READSCOV — "없는 키 0" 의 분모 (`D-V3-FINDING-099`, D-DEF-109)

직전 회차의 `없는 키 0` 은 **전체 읽기 272 중 139** 만 본 값이었다 — **사각이 절반**.
...
원인: **`as` 없는 임포트**를 못 잡음(`ctl`·`au` 통째로 빠짐) + **중첩 한 단계** 미해석.

**`없는 키 0` 은 무엇 중의 0인지 밝히지 않으면 안심용 숫자다.** `D-DEF-79`(전수라 해 놓고
분모가 손 목록)와 같은 형태 — **적었으되 세지 않았다.**

**미결**: 미해석 21(튜플 언패킹 일부·두 단계 이상 중첩). `d_bus_scan.sh` 한 파일만 본다.

## RQ-D-BUS-025 — `T-B-V3-FINDING-025` (cc=D) · ACK

B 가 **내 limitation 이 가리킨 자리**를 찾았다 — `nav_container_type.AMBIGUOUS_MULTIPLE_CONTAINERS`
1행(`F3-02`). 재계산 일치. **D 화이트리스트는 그 토큰을 처음부터 제외**하고 있었다.

**사이드카 안에서 두 수가 어긋난다**:
- 구조화 `per_column.entry_control_type.n_observed` = **27** (D 와 일치 → `D-097` DISAGREE 해소)
- 산문 블록 `두 분해가 다른 이유` = **28** (미반영)

**새로 올린 것**: nav 축의 **또 다른 정의 차** — D **26** vs 사이드카 **16**, 차이 **10 = `NONE`**.
`AMBIGUOUS_MULTIPLE_CONTAINERS` 1건보다 **10배 크다**. **A 결정 사항**.
...
## RQ-D-AXIS-RESOLVED — `D-100` 의 3축 17셀 해소 (`D-V3-FINDING-101`, D-DEF-112)

`D-100`(P1) 발행 **2분 뒤** 사이드카 갱신으로 **4축 19셀 → 1축 2셀**.

| 축 | 전 | 후 | 무엇 |
|---|---|---|---|
| `nav_container_type` | 16 | **26** | `NONE` 제거 — **D 입장 채택** |
| `auth_gate_stage` | 2 | **8** | `NONE` 제거 — **D 입장 채택** |
| `label_relation` | 1 | **0** | `VISIBLE_ONLY` 추가 — **C 정정 반영** |
| `terminal_reason` | 50 vs 48 | **그대로** | `..._UNVERIFIED_CANDIDATE_COUNT` 2셀 — **A 판단** |

**mart CSV 는 바이트 불변**(`5290e0c3…`) — 사이드카만 바뀌었다.

**전수 대조의 값**: 한 질문(`NONE`)이 **두 축에 걸쳐** 있었다 — 축별로 갔으면 두 번 만났다.
