...
W3 는 실행하지 않고 보고했다
출처   Research Director 의 입력이 W3 스레드로 전달된 것 (B 가 T-B-FC-009 로 규명)
확정   **Director 확인 — 오배송이다. 의도된 지시가 아니었다** (00:00 KST)
```
...
### D-R0-81-4 — 오배송으로 확정됐다

**Director 가 확인했다 — 이것은 오배송이며 의도된 지시가 아니었다.**

```
따라서   '정당한 권한자가 경로 밖으로 지시했다' 가 아니라
         '의도되지 않은 메시지가 worker 스레드에 들어갔다' 이다
그리고   그것이 D-R0-81 을 약화시키지 않고 강화한다
```

```
의도된 지시였다면   출처 확인 후 집행하면 됐다
오배송이었으므로    실행됐다면 아무도 의도하지 않은 REAL_TARGET 수집이 일어날 뻔했다
                    — W3 의 거절만이 그것을 막았다
```

**A 는 Director 에게 절차 변경을 요구하지 않는다.** 오배송은 절차 위반이 아니라 사고다.
`D-R0-81-1~3` 은 그 사고가 결과로 이어지지 않게 하는 수신 측 규칙이며, 그것으로 충분하다.
