...
### 그러나 이 주장은 검증 불가능하다 — 그 사실을 함께 적는다

독립 검산자(C)가 정확히 지적했다: **위 문장은 A 의 진술이며 C 에게 검증 수단이 없다.**
그리고 **그 값들은 E000 batch result 에 열람 가능한 상태로 존재했다.** 내가 보지 않았다는 것은
증거가 아니라 자기보고다. 이 문서의 어떤 부분도 그 주장 위에 서 있어서는 안 된다.

**대신 구조적으로 확인 가능한 것을 남긴다.** 결과를 보고 분석을 고른 개정이라면
다음 흔적이 남았을 것이다:

| 만약 결과를 보고 골랐다면 | 이 문서의 실제 상태 |
|---|---|
| obstruction 변수를 **특정해서** 지정했을 것 | **자동 선택 규칙 유지** (결측률 최소, 상관계수는 선택 로직에 입력 금지) — 내가 고를 수 없다 |
| 예상 방향(양/음)을 시사했을 것 | **방향을 명시하지 않았다** |
| 유리한 임계·최소 N 을 설정했을 것 | **모문서 임계 그대로** (`pairwise-complete n < 10` → exploratory only) |
| `joint_valid_n` 을 살리려 J3 를 완화했을 것 | **J3 무변경**, 낮은 값 그대로 보고 (§4) |
| 강등 규칙을 느슨하게 했을 것 | **2축 부호 안정성 · GRADE C 강등 그대로** (§5) |

**이것은 타임스탬프보다 약한 증거다.** 다만 검산자가 문서 자체에서 확인할 수 있다.
C 는 이 표의 각 행을 실제 문서·코드와 대조하는 것으로 A 의 자기보고를 부분적으로 대체할 수 있다.

**향후 개선:** 분석 계약은 **evidence 가 하나도 존재하지 않는 시점**에 동결돼야 이런 모호함이 없다.
모문서(`LA-AC-20260827`)는 그 조건을 만족했으나 본 개정은 만족하지 못한다. post-E001 backlog.

이후 이 개정을 다시 고치면 §6 변경이력에 "데이터 관측 이후인가" 를 반드시 채운다.
