...
## Δ50 — 두 층은 층이 아니었다. **같은 문서를 읽는 두 검사는 하나다** (C-ASSURANCE-090716)

`C` 가 `T-A-V3-FC-008` 요청 ① 을 독립 확인했다. 방법: scratch clone 에 `origin/control/...` 을 철회 **전**(`7752ed79`)과 **후**(`fd304ef0`)로 각각 세우고 두 층을 SUT 에서 import 해 판정식을 호출.

| scope | 철회 전 | 철회 후 |
|---|---|---|
| `E000_FAST` | **L1 `ALLOW` + L2 `PASS`(`'REAL_TARGET'` 반환)** | L1 `DENY` + L2 `BLOCK` |
| `E001_FULL` | `DENY`/`BLOCK` | 동일 |
| `V2_DIAGNOSTIC` | L2 가 아는 scope 밖 → `BLOCK` | 동일 |

SUT 2종(v2.1 참조 `01041bc` · `B` 최신 `a4185e4c`) 모두 같다.

**`A` 가 잰 것보다 나빴다.** `A` 는 배치층 판정식만 쟀고 "두 층의 연언까지는 확인하지 않았다" 고 적었다.
**연언도 열려 있었다.** 철회 전 `E000_FAST` 는 실제로 통과하는 상태였다.

`A` 가 그때 **모르는 것을 모른다고 적은 것**이 옳았다. 그 자리를 비워 뒀기 때문에 `C` 가 채웠고, 채워진 값이 더 나빴다.

### R48 — 다층 방어는 **입력이 다를 때만** 층이다

`C` 가 적은 문장이 이 절의 전부다.

> 두 층이 **같은 문서를 읽으므로** 문서 하나가 열리면 둘 다 열린다 —
> **연언이 방어가 아니라 같은 입력의 두 독법이다.**

`layer_firewall.py` 의 docstring 은 독립성을 약속한다:

> `scope` 가 주어져도 이 층이 **자기 힘으로** 릴리스 문서를 읽어 승인 여부를 다시 확인한다 — 엔진 층이 통과시켰다는 사실은 이 층의 판정 근거가 아니다.

**그 문장은 판정의 독립성에 대해서는 참이고, 방어의 독립성에 대해서는 거짓이다.**
다른 층의 *판정*을 믿지 않는 것은 맞다. 그러나 같은 *입력*을 믿는다. 그래서 층이 둘이 아니다.

`Δ41-R36`(문서도 주장이다)이 걸린다 — docstring 이 실제보다 강한 것을 약속한다.
그리고 `Δ49-R47` 이 코드에 적용된 형태다: **같은 출처를 읽은 두 일치는 하나다.**

**규칙**: 다층으로 부르려면 **각 층의 입력이 서로 다른 것**이어야 한다. 아니면 층 수를 세지 말고 **단일 지점**이라고 적는다.

### 이 저장소에 이미 답이 있다

`V2_DIAGNOSTIC` scope 는 그렇게 만들어져 있었다 — 릴리스 문서가 `manifest_sha256` 을 `DIAGNOSTIC_PILOT_MANIFEST_SHA256` 과 **동일하게** 싣고 있어야 열린다.

> scope 승인과 표본을 묶는 바인딩이다(**표본이 조용히 바뀐 릴리스 문서로 다른 12 target 을 여는 것을 막는다**).

**`E000_FAST` 에는 그 바인딩이 없었다.** 릴리스 문서 하나로 열렸다.

### `V3_PILOT_5` release 의 설계 조건 — 추가

`Δ19-R10`(`preconditions[]` 에 exact SHA)에 더한다.

| 조건 | 내용 |
|---|---|
| 입력 분리 | **두 번째 층은 릴리스 문서가 아닌 입력을 읽는다** — 동결 manifest 의 sha 바인딩(`V2_DIAGNOSTIC` 방식) |
| 실증 | 릴리스 문서만 고치고 manifest 를 그대로 두면 **열리지 않음**을 `must_not_flag` 로 보인다 |
| 반대 | 둘 다 맞을 때만 열림을 `must_flag` 로 보인다 |
| 표기 | 층 수를 세는 모든 서술에 **각 층의 입력**을 함께 적는다 |

### SUT 확정 — `A` 가 답한다

`C` 가 물었다: promoted main `bc0b7a08` 에는 firewall 모듈이 없어 SUT 를 `B` 러너 계열로 잡았는데, **어느 SHA 가 현재 SUT 인가.**

**`GATE 1` 의 SUT 는 `B` 의 `COMPLETION` SHA 로 확정된다.** 그 전까지는 SUT 가 미확정이다.
`C` 가 두 계열(v2.1 참조 · `B` 최신 push)을 **모두** 검사한 것이 옳고, 그 결과에는 **`SUT_미확정` 표기**를 단다.
`COMPLETION` 이 오면 그 SHA 에서 다시 잰다 — **미확정 상태의 통과는 확정 상태의 통과가 아니다.**

### `C` 자기정정 — 세 번째 평면에서 같은 것

`C` 의 probe 가 **모듈 import 실패를 `fail-closed` 로 표기**하고 있었다. `Δ46-exit2` 대로 `exit 2` "돌지 않았다" 로 고쳤고, **첫 두 실행이 그 상태였으므로 그때 값은 근거로 쓰지 않는다.**

`A`(검사기) · `B`(`r32_check`) · `C`(probe) **세 평면 전부에서 같은 결함이 나왔다.**
`Δ46-exit2` 가 규약 하나를 고친 것이 아니라 **공통 오해 하나를 고친 것**이었다 — 검사가 못 돈 것을 "안전하게 막혔다" 로 읽는 오해.

---

## Δ51 — REAL 은 **어디를 쳤는지**로 판정한다. 그리고 소비자가 0인 안전 플래그는 보호처럼 읽힌다 (C-ASSURANCE-091613)

`C` 가 `FC-008` 요청 ② 를 `Δ40-R34`(b) 로 수행했다 — 정적 AST/grep 열거, 실행 0, 48 지점.
`C` 의 시점 표기가 정확하다: `A` 의 진술에 대해서는 **informed**, 목록 자체는 **시점독립**(아무 평면도 이 단위를 공표한 적이 없다).

### 판정 ① — 무게이트 네트워크 접근 3종

**`REAL` 여부는 무엇을 하러 갔는지가 아니라 어디를 쳤는지로 판정한다.**

| 스크립트 | 어디를 치는가 | 판정 |
|---|---|---|
| `shadow/lane_b/scripts/probe_official_urls_shadow.py` | **연구 대상 공식 서비스 URL** 을 모바일/데스크톱 UA 로 `GET` | **`REAL` 이다** |
| `scripts/acquire_wiseapp_authority.py --acquire` | 권위 **목록 페이지** | `REAL` 아님 |
| `scripts/collect_certification_registry.py` → `registry.crawl()` | **인증목록** 페이지 | `REAL` 아님 |

`probe_official_urls_shadow.py` 는 **관측 대상 도메인에 요청한다.** Director 가 이미 그 범주를 다뤘다 —
모바일 UA `L0` precheck 는 **`A` 가 eligibility-precheck scope 를 release 해야** 가능하고, 그때도 **task click/input/flow 실행 금지**다.

**따라서 이 스크립트는 현재 실행 금지다.** 게이트를 붙이거나, 붙기 전까지 실행 불가 상태로 둔다.
"shadow" 라는 이름이 게이트가 아니다. **디렉터리 이름은 방어가 아니다.**

나머지 둘은 **표본을 구성한 출처**이지 관측 대상이 아니다(이 코호트가 그렇게 만들어졌다).
그러나 **무게이트로 두지 않는다** — 얕은 조건 둘:

1. 실행 시 **요청 호스트가 목록/권위 출처 allowlist 안**임을 검사한다. 대상 서비스 도메인이 나오면 차단
2. 실행 기록을 남긴다

이유: **같은 스크립트가 확장되어 대상 도메인을 치는 일은 조용히 일어난다.** 지금 안 치는 것은 현재 코드의 성질이지 계약이 아니다.

### R49 — `REAL` 판정 기준은 **요청 대상 호스트**다

의도·모드·디렉터리·스크립트 이름이 아니라 **네트워크 요청이 어느 호스트로 나갔는가**로 판정한다.
`REAL` 게이트는 그 기준으로 걸린다. 관측 대상 도메인 집합은 **동결 manifest 에서 파생**한다 — 별도로 손으로 적지 않는다.

### 판정 ② — 고아 플래그를 소비시킨다. 그것이 `Δ50` 의 두 번째 입력이다

`C` 실측: `FINAL_MAIN50_MANIFEST.json` 의 `real_target_allowed=false` 는 `v3_runner/registry.py:1153` 이 파싱하지만 **소비자 0** 이다.

**소비자가 0인 안전 플래그는 보호처럼 읽힌다.** 문서를 읽는 사람은 그 값이 무언가를 막는다고 생각한다.
`R41`("항상 참인 필드는 아무것도 말하지 않는다")·`R43`("이름만 남기지 않는다")의 데이터판이다.

**R50 — 소비자가 0인 안전 플래그는 소비시키거나 제거한다.** 남겨 두지 않는다.

그리고 이것이 `Δ50-pilot5-binding` 이 찾던 **두 번째 입력**이다.

| 두 번째 층이 읽는 것 | 조건 |
|---|---|
| `FINAL_MAIN50_MANIFEST.json` | (a) 자기 `manifest_sha256` 이 파일 내용과 일치 (b) `real_target_allowed is True` (c) **target 이 동결 50 안에 있다** |
| 릴리스 문서 | 그 manifest 의 sha 를 싣고 있고 **일치한다** |

`V2_DIAGNOSTIC` 이 이미 쓰던 방식이다. **릴리스 문서만 고쳐서는 열리지 않는다** — 그것이 `must_not_flag` 다.

### 판정 ③ — `E000_FAST_PLAN` allowlist

`E000_FAST` 는 철회됐으므로 **sha 고정에 투자하지 않는다.** 다시 열려면 새 release 문서가 필요하고, **그 문서가 pin 을 실어야 한다**(`R50` 의 바인딩과 같은 조건).

다만 `C` 가 짚은 것 하나는 지금 고친다: allowlist 의 두 번째 후보 경로가 **트리 밖 형제 워크트리**를 가리킨다.

**저장소 트리 밖으로 해소되는 allowlist 경로는 fallback 이 아니라 hard error 다.**
바깥 경로가 조용히 잡히면 **어느 파일이 표본을 정했는지가 저장소에 남지 않는다.**

### `batch.py` 를 건너뛰는 경로 — 게이트를 아래로 내린다

`C` 실측: `batch.py:607-609` 에서 두 assert 가 순차로 돈다(연언 실재). 그러나
`L0Collector`/`run_l0_real` 을 **직접 생성**하면 `layer_firewall` 을 건너뛴다(engine 게이트는 남음).

`R48` 로 이미 두 층이 하나였으므로, **실효 게이트는 engine 하나**다. 따라서:

> **`Δ50-pilot5-binding` 의 두 번째 입력 검사는 `batch.py` 가 아니라 engine 게이트에 건다.**

진입점에만 걸면 진입점을 우회하는 호출이 그대로 지나간다. `C` 가 그 경로를 이미 열거했다.

### `scope_mismatch` — 접수

`layer_firewall` 은 `{E000_FAST, E001_FULL}` 만 알고 engine 은 `V2_DIAGNOSTIC` 도 안다.
→ `run_v2_diagnostic_pilot.py` 는 `V2_DIAGNOSTIC_RELEASE.json` 과 무관하게 **배치층에서 항상 차단**된다.

현재는 `V2_DIAGNOSTIC` 이 철회 상태이므로 **결과가 안전 방향**이다. 그러나 **이유가 우연이다** — 두 층이 아는 scope 집합이 다르다는 것이지 정책이 아니다.
`Δ47-refusal` 과 같은 자리다: **막힌 것과 막도록 만든 것은 다르다.** 기록만 하고 지금 고치지 않는다.

### `C` 가 적은 미탐색

`C` 가 **탐색하지 않은 것**을 열거했다 — 다른 워크트리·작업 트리·`research/landing_accessibility` 밖(`.claude/settings.json` env 포함)·`l0_probe.js`·바이너리·이전 ref 이력·`MLflow` 서버.
**이것이 `Δ40` 이 요구한 형태다** — "전수" 라고 쓰지 않고 무엇의 전수인지 적었다. `engine/` 을 "0건이 아니라 미탐색" 으로 적은 `B` 와 같다.

---

## Δ52 — `A` 의 버스 미러는 **부분이었고 실패가 조용했다** (D-V3-FINDING-023.C ACK 파급)

`C` 가 `D` 의 자기정정을 받아 **자기 스캐너에서 같은 형태**를 찾았다 — 손상된 ACK 파일이 `except: continue` 로 넘어가 `parse_errors` 에 오르지 않고, **파일 존재만으로 acked 집합에 들어갔다.**

`A` 가 자기 미러에 같은 검사를 걸었다.

| | 버스 | 미러 | |
|---|---|---|---|
| `tickets` | 286 | **109** | 미복사 177 |
| `acks` | 659 | **270** | 미복사 389 |
| `completions` | 39 | **0** | 미복사 39 |

**`A` 는 자기가 만진 것만 옮겼고, `cp ... 2>/dev/null` 로 실패를 삼켰다.**
실패한 복사는 출력이 없어 성공과 구분되지 않는다 — 이 세션의 중심 결함이 `A` 의 미러 명령 한 줄에 있었다.

내용 불일치는 **0** 이었고 미러에만 있는 파일도 **0** 이었다. 즉 **틀린 사본은 없었고 없는 사본이 많았다.**
그러나 `A` 는 이 미러를 **복구 근거**로 인용한 적이 있다(중복 `ticket_id` 사건에서 `21e0a48` 로 v1 을 되살렸다).
그 복구가 가능했던 것은 **그 티켓이 마침 복사돼 있었기 때문**이지 미러가 그것을 보장했기 때문이 아니다.

> **선택적 복사는 `A` 가 무엇을 봤는지를 남기지만, 무엇이 있었는지는 남기지 않는다.**

### R51 — 미러·사본·색인은 **측정된 동기화**여야 한다

| 요구 | 내용 |
|---|---|
| 전량 | 자기가 만진 것이 아니라 **원본 전량**을 대상으로 한다 |
| 검증 | 복사 후 **다시 재서** 일치를 확인한다. **복사 성공을 가정하지 않는다** |
| 조용한 실패 금지 | `2>/dev/null` 로 오류를 삼키지 않는다 |
| 세 방향 | 없는 것 · **다른 것** · **원본에 없는데 사본에 있는 것** 을 각각 센다 |
| 미실행 | 원본을 못 찾으면 `exit 2` — **"사본이 비었다" 가 아니라 "검사가 돌지 않았다"** |

시정: `control/mirror_bus.py`. `--sync` 후 **재검증**까지 하고, 재검증에서 불일치가 남으면 `exit 1`.
`must_flag`(동기화 전) `exit 1` · `must_not_flag`(동기화 후) `exit 0` 실측.

### `exit 2` 규약이 첫 실사용에서 값을 했다

`mirror_bus.py` 첫 판이 버스를 **워크트리 루트에서만** 찾았다. `A` 는 워크트리에서 돌고 버스는 **메인 저장소**에 있다.

그 결과가 **`exit 2` + "버스를 찾지 못했다 — 검사가 돌지 않았다. 통과로 읽지 마라"** 였다.

만약 `Δ46-exit2` 이전의 규약이었다면 이 도구는 **디렉터리가 없으니 파일 0개 → 불일치 0 → 통과**를 냈을 것이다.
**빈 원본과 일치가 같은 출력이 되는 자리였고, 그것을 막은 것이 바로 그 규약이다.**

`Δ50-exit2-common` 이 "규약 하나가 아니라 공통 오해를 고친 것" 이라고 했는데, **그 규약이 만들어진 뒤 처음 실제로 발화한 곳이 `A` 자신이었다.**

---

## Δ53 — `Δ48-R43` 의 예시가 틀렸다. 그리고 워커가 `R43` 보다 날카로운 축을 찾았다 (T-B-V3-FC-005)

### `A` 자기오류 — 세 번째로 `B` 의 성격규정을 검증 없이 인용했다

`Δ48` 에 `A` 가 이렇게 썼다:

> `verify_retention_manifest` 가 **아무것도 검증하지 않고 `ok: True`** 를 내므로 이 단언은 깨질 수 없다.

**틀렸다.** `B` 의 `R43` 전수 실증 결과:

- 실패 입력 **3종**(sha 변조 · 디스크 부재 path · aggregate 변조) → 전부 `{"ok": false}`. **`CAN_FAIL` 이다**
- `git log -L625,655` — 도입 커밋 `e8d8829` 이후 **한 번도 바뀌지 않았다.** `B` 가 본 시점에도 같은 코드였다
- **`manifest={}` 이면 `roots` 순회가 0회**라 무엇도 대조하지 않고 `ok: true` 가 나온다

> **함수는 검증한다. 그 호출이 검증할 것을 주지 않았다.**

`A` 는 `B` 의 headline 을 그대로 옮겨 `R43` 의 근거로 삼았다. `R17` 을 또 어겼다.
`B` 의 핵심 지적("**그 테스트가 결함을 가렸다**")은 **유효하고**, 틀린 것은 **가려진 결함의 이름**이다.
`R43` 자체도 유효하다 — **다만 이 사례가 그 규칙의 예시로는 부정확했다.**

### R52 — `VACUOUS_PASS` 를 별도 범주로 승격한다

워커가 축을 둘로 갈랐다.

| 축 | 질문 |
|---|---|
| `CAN_FAIL` / `CANNOT_FAIL` | 이 함수는 **실패할 수 있는가** |
| **`VACUOUS_PASS`** | 이 호출은 **검사 0회로 성공을 내는가** |

워커의 문장이 `A` 의 규칙보다 정확하다:

> `Δ48` 이 실제로 본 결함은 '실패 입력이 없다' 가 아니라 **'검사 0회로 성공을 낸다'** 였고 … **축 1 만 재면 놓친다.**

**"검사했고 통과" 와 "검사할 것이 없어서 통과" 가 같은 출력이다.**
`Δ46` 이 검사기 `exit` 에서 잡은 것과 같은 층이고, **이번엔 검사기 안이 아니라 검사 대상 안**이다.

**`R43` 을 확장한다**: 실패 가능성 실증은 **함수 수준으로 끝나지 않는다.**
`CAN_FAIL` 함수라도 **호출 지점의 입력이 실패를 유발할 수 없으면** 그 호출 지점은 깨진 검사기와 같은 출력을 낸다.
따라서 실증은 **호출 지점의 실제 입력**에서 한다. `VACUOUS_PASS` 실측 **2건**은 `P1` 시정 대상이다.

### R53 — 지시문의 사실 주장이 틀리면 **게이트를 깨지 말고 `FALSIFIED` 로 기록한다**

`B` 가 워커 지시문에 두 가지를 사실처럼 넣었고 **둘 다 틀렸다.**

| `B` 의 지정 | 실측 |
|---|---|
| `verify_retention_manifest` 를 `CANNOT_FAIL` 로 잡아라(`must_flag`) | **`CAN_FAIL`** |
| `assert_search_strategy_declared` 를 `must_not_flag` 로 | **`v3_runner` 전수 0건** — 다른 브랜치(W5O) 산출 |

워커의 처리가 **정확히 옳다**:

- `must_flag` 가 틀린 것을 **게이트 실패로 만들지 않고** `briefed_control.status: "FALSIFIED"` 로 기록했다 — **방법이 아니라 저장소 상태에 대한 주장이었기 때문**이다
- 도구가 `CANNOT_FAIL` 을 잡는 능력은 **`SYNTHETIC::verify_always_ok`** 로 별도 실증했다
- 없는 함수는 `evidence.py::assert_layer_qualified` 로 **대체하고 문서에 적었다**

**규칙**: 지시문의 사실 주장이 반증되면 **게이트를 깨지 않는다.** 게이트를 깨면 **방법의 결함과 지시의 결함이 같은 출력**이 된다.
`FALSIFIED` 로 기록하고, **도구의 능력은 합성 대조로 따로 실증**한다.

그리고 `B` 가 자기 형태를 적었다 — **지시문에 자기 추론을 사실처럼 넣은 것이 세 번째**(`FC-013`·`FC-015`·이번), **세 번 다 워커가 실측으로 잡았다.**
`A` → `B` 도 같다. `A` 가 `B` 의 성격규정을 인용해 `Δ48` 을 쓴 것이 그 층의 같은 사건이다.
**상위가 하위에 내리는 사실 주장은 하위가 반증할 수 있어야 하고, 상위는 그 반증을 게이트가 아니라 기록으로 받는다.**

### 판정 ③ — `engine/` 27건의 실증 소유

`B` 는 `engine/` 27건을 **열거만** 하고 `ENUMERATED_NOT_DEMONSTRATED` 로 적었다. **"0건이 아니라 미실증" 이라고 적은 것이 옳다.**

**판정: `v3` 가 실제로 호출하는 부분집합만 `B` 가 실증한다. 나머지는 `ENUMERATED_NOT_DEMONSTRATED` 로 남기고 그 표기를 `GATE 1` 의 "검증하지 않은 것" 절로 옮긴다.**

근거: `v3` 의 주장은 **`v3` 가 실행하는 경로**에 의존한다. `v3` 가 부르지 않는 engine 함수의 실패 가능성을 실증하는 것은 `v3` 주장에 아무것도 더하지 않으면서 비용이 크다.
`Δ43` 에서 BFS 를 선제 도입하지 않고 `pilot` 관측으로 미룬 것과 같은 판단이다.

그리고 `B` 가 적은 한계가 유지된다 — **27 은 워커 규칙 기준의 수이고, 다른 평면이 다른 규칙으로 세면 다른 수가 나온다**(`Δ49-R45` 의 단위 문제).

### 확인 — `B` 가 옳게 한 것 셋

1. **`R36` 이름 검사가 실제로 하나를 잡았다.** 첫 판 `probe_target_removed_from_source` 가 함수를 통째로 잘라 상수 블록까지 없애 관측이 **`exit 2`(미실행)** 였다 — **이름이 주장한 `[표류]` 는 일어나지 않았다.** 변형을 좁히고 이름을 `probe_target_renamed_in_source` 로 고쳤다
2. **`data_mutation_leaves_controls_intact`** — `A` 가 `Δ46` 에서 겪은 것을 **이름 그대로 실증**했다. 데이터만으로는 대조군을 못 깬다
3. **`R40` 결속이 informative 하다** — 검사기에 **주석 한 줄**만 추가해도 `valid_for_this_commit: false`. `binding_is_informative: true` 를 실측으로 붙였다. **항상 참인 필드가 아니다**

### 부기 — 검사기가 자기를 잡은 것

워커가 도구를 `v3_runner/` 아래 두자 `r32_check.sweep_candidates` 가 **자기 도구를 `R32` 미등재 후보로 잡아 실패**했다.
`r32_check.py` 수정이 금지라 `r43/` 하위로 비켰다.

**검사기가 검사기를 잡는 것은 정상 동작이고, 배치로 푼 것이 맞다.** 검사 규칙을 느슨하게 해서 자기를 빼는 것이 오답이다.

---

## Δ54 — 좁은 매처가 낸 `0` 이 이 세션에서 가장 자주 나온 결함이다 (D-V3-FINDING-025)

`D` 의 `MLflow` 색인이 `FINDINGS.md` 를 가진 **27건 중 18건**의 한계를 **빈 값**으로 기록해 왔다.
그 18건은 **전부 한계를 적고 있었다** — 파서가 **번호 붙은 표제**(`## 12. Limitation`)를 못 잡았다.

**빈 값과 없음이 같은 출력이었다.** 그리고 빈 쪽이 "한계 없음" 으로 읽힌다.

### 같은 형태가 이 세션에서 몇 번째인가

| 사건 | 좁았던 것 | 결과 |
|---|---|---|
| `A` 의 절 표제 정규식 | 매칭 패턴 | `delta_section_coverage` 가 51/63 만 보고 통과 |
| `D` 의 별칭 필터 | 모양 기준 | 가장 특정한 별칭을 버려 도달 불가 9 vs 8 |
| `B` 의 `R32` 열거 단위 | 매개변수 기준 | 24건(과다) → 2건(과소, 시작 사례 누락) |
| `B` 의 `OUT` 측정 | 코드 경로 하나 | `must_flag` 를 `R32_OK` 로 오판 |
| **`D` 의 한계 파서** | 표제 형식 | **18/27 을 빈 값으로** |

**다섯 번이다. 이 세션 최다 결함 유형이다.**
그리고 다섯 번 다 **출력이 정상으로 보였다** — 좁은 매처는 오류를 내지 않고 `0` 을 낸다.

### R54 — 추출기는 **'없음' 과 '못 뽑음' 을 구분**해야 한다

`R32` 의 세 상태가 파싱에 적용된 형태다. `C` 가 `D` 의 ACK 에서 이미 같은 결론을 적었다 —
`NOT_STATED`(**파서가 못 찾음, 파서 규칙 병기**) / `EMPTY`(절은 있고 내용이 빔) / 값.

그리고 **탐지 방법**이 규칙보다 중요하다:

> **추출 결과가 빈 비율을 원문의 존재 여부와 대조한다.**
> `D` 의 경우 "27건이 `FINDINGS.md` 를 갖는데 18건이 빈 값" 이 **한 번의 대조로 드러났다.**
> 분모 없이 빈 값만 보면 그것은 관측처럼 보인다.

`C` 의 추가 규칙도 채택한다 — **한계는 파생 색인이 아니라 인용 sha 의 산출 바이트에서 읽는다.** 색인은 사본이고 사본은 갈린다(`Δ38`·`Δ52`).

### `A` 자기적용 — 검사가 통과하는 이유가 검사 범위 밖에 있었다

`A` 가 자기 절 매처를 실측했다: 전체 `##`/`###` 표제 **234** 중 절 id 로 잡히는 것 **103**, 못 잡는 것 **121**.

121 은 대부분 산문 소절이라 문제가 아니다. **문제는 다른 데 있었다.**

`A` 는 판정을 **번호 소절**(`### ① … ④`)에 담는다 — `Δ36` 의 ①②③④ 가 각각 독립 판정이다.
그 소절들은 색인에 `Δ36-order` · `Δ36-budgetseam` · `Δ36-tokens` 로 **들어가 있다.**
그러나 **`delta_section_coverage` 는 그것을 확인할 수 없다** — 절 표제만 보기 때문이다.

> **피복은 유지되고 있었지만, 유지한 것은 검사기가 아니라 손이었다.**
> 검사가 통과한 이유가 검사 범위 밖에 있으면 그 통과는 검사의 결과가 아니다.

### 시정 — 10번째 검사 `numbered_subsection_coverage`

각 `Δn` 절의 **번호 소절 수**를 세고, 그 authority 의 **색인 행 수가 그보다 적으면 `FAIL`**.

**`must_flag` 실증**: `Δ36` 의 행을 전부 지우고 격리 사본에서 실행 →
`FAIL numbered_subsection_coverage: {'Δ36': {'번호소절': 4, '색인행': 0}}` · `exit 1`.

#### 그리고 이 검사의 양성대조가 첫 판에 **못 잡았다**

probe 가 "행이 가장 많은 authority" 를 골랐는데 **그 절에 번호 소절이 0개**여서 변이가 아무것도 깨지 않았다.
**probe 가 자기가 겨냥한 검사와 다른 곳을 쳤다.**

검사기는 그때 `ok=False` 를 내고 **`exit 2` 로 기록을 거부했다** — "양성대조 실패, 아래 결과를 근거로 쓰지 마라".
**새 검사를 넣자마자 그 검사의 대조가 무력했고, 그것을 검사기가 스스로 막았다.**
`Δ34`(서술이 아니라 실행 가능한 검사기)와 `Δ46`(`exit 2`)이 함께 작동한 자리다.

`probe` 를 **번호 소절을 실제로 가진 authority** 를 고르도록 고쳤다. 현재 **10/10 통과 · 변이 10/10**.

---

## Δ55 — 정본이 있는데 파생이 다른 것을 읽으면, **정본의 존재가 방어가 아니다** (D-V3-FINDING-026)

`D` 가 `FINDING-025` 직후 **이웃 필드도 같은 파일을 같은 방식으로 읽는지** 확인했다. verdict 는 더 나빴다.

| | |
|---|---|
| 색인된 `RQ` | 36 |
| **정본과 어긋남** | **18** |
| **방향 뒤집힘** | **2** — `D-SUP-02` 색인 `NOT_SUPPORTED`/정본 `SUPPORTED` · `RQ-D12A` 색인 `SUPPORTED`/정본 `REFUTED` |

기전: `parse_verdict(md)` 가 `sorted(VERDICTS, key=len, reverse=True)` 로 **앞 4000자에서 가장 긴 토큰**을 고른다.
가설별 판정표에 `PARTIALLY_SUPPORTED` 가 먼저 나오면 그것이 `RQ` 의 판정이 되고, 판정이 창 밖이면 `UNKNOWN`(12건).

**좁은/휴리스틱 매처가 낸 잘못된 값이 여섯 번째다**(`Δ54` 의 다섯 + 이번). 이번은 `0` 이 아니라 **반대 값**이라 더 나쁘다.

### 가장 무거운 문장

> **완결 게이트가 이미 JSON 최상위 `verdict` 를 요구한다. 색인만 다른 것을 읽고 있었다.**

정본은 정해져 있었고 게이트도 있었다. **파생이 정본을 안 읽었을 뿐이다.**
`Δ38`(측정값은 한 곳에)·`Δ52`(사본은 갈린다)가 값에 대한 규칙이었다면, 이것은 **읽는 경로**에 대한 규칙이다.

### R55 — 파생 산출은 **정본 필드**를 읽는다. 재구성하지 않는다

| 요구 | 내용 |
|---|---|
| 경로 | 정본 필드가 있으면 **그것만** 읽는다. 산문에서 재구성하지 않는다 |
| 대체 | 정본이 없어 대체 경로를 탔으면 **`*_source` 를 함께 싣는다**(`D` 가 한 대로: 33/36 `json_top_level` · 3 `md_heuristic_fallback`) |
| 금지 | "가장 긴 토큰" · "앞 N자" 같은 **창·순위 휴리스틱**으로 판정값을 뽑지 않는다 |
| 대조 | 정본 우선(`must_flag`) · 정본 없을 때 대체 · 구조형 — 세 갈래를 실증한다 |

### 판정 — 기존 run 을 어떻게 하는가 (`D` 가 `A` 에게 올린 것)

`D` 가 정확히 물었다: **불변성 규약과 인용 무효화 사이의 판단**이다.

**덮어쓰지 않는다.** `Δ41-R36` — 기록을 고치면 그것을 인용한 문서가 조용히 근거를 잃는다.
**그러나 그대로 두는 것도 답이 아니다.** 잘못된 18건이 색인에 살아 있고, **결과 sha 가 같아 sync 가 재기록하지 않으므로 자동으로 고쳐지지 않는다.** 그러면 이 티켓만이 유일한 경고가 되고 **티켓은 흐른다.**

**판정: 제3안 — 값은 고치지 않고 무효 표시를 붙인 뒤 새 run 을 추가한다.**

1. 기존 run 에 태그를 **추가**한다 — `verdict_index_defect: D-V3-FINDING-026` · `superseded_by: <new_run_id>`
2. 새 run 이 `authoritative_verdict` 로 기록한다
3. 색인 소비자는 `superseded_by` 가 있는 run 의 verdict 를 **인용하지 않는다**

**불변성은 값을 고치지 않는 것이지 메타를 못 붙이는 것이 아니다.** 태그 추가는 기록을 지우지 않는다.

### R56 — 재계산 판단은 (입력 sha, **도구 sha**) 쌍으로 한다

`D` 가 짚은 것이 이 절보다 오래 간다:

> 결과 sha 가 같아 sync 가 재기록하지 않으므로 **자동으로 고쳐지지도 않는다.**

**도구를 고쳤는데 입력이 같다고 건너뛰면, 도구 수정이 산출에 도달하지 않는다.**
`Δ44-R38` 은 "측정값에 입력 신원을 붙이라" 였다. 그 뒷면이 여기 있다 — **입력 신원만 보고 도구 신원을 보지 않으면 재계산이 누락된다.**

`A` 의 `Δ46-R40`(실증을 도구 sha 에 묶는다)과 같은 재료다. **캐시·sync·색인 갱신의 키는 `(input_sha, tool_sha)` 다.**

### 방화벽이 작동했다 — 그리고 `A` 가 그것을 실측했다

`D` 가 발행한 pointer-only `FINDING` 은 **JSON 정본을 인용했다.** 티켓의 판정은 옳고 **틀린 것은 색인이다.**

`A` 도 자기 인용을 쟀다: delta 전문에서 `D-SUP`·`RQ-D`·`SUPPORTED`·`REFUTED` 매칭 **1건**이고,
그것은 `RQ-D13b` 를 **`D` 의 자기정정 사례**로 인용한 것이지 verdict 를 근거로 쓴 것이 아니다(`H1_NO_EFFECT` 언급 1건도 형태 비유다).

**`v3` 주장 사슬에 `D` verdict 는 실려 있지 않다.** `D` 가 `NON_CANONICAL` 이라는 지위가 이 결함의 영향을 이미 제한했다 —
**그 지위가 장식이 아니었음이 이번에 실측으로 확인됐다.**

---

## Δ56 — `A` 의 추론 셋이 base 실측으로 확증됐다. 그리고 **만들어진 것과 배선된 것**을 가른다 (T-B-V3-BLK-018)

`B` 가 소스만 base 판본으로 되돌려(`git stash push -- <파일>`) **행동을 직접 측정**했다.
**심볼 부재로 인한 `ImportError` 를 근거로 쓰지 않았다** — `R31`·`R46` 이 요구하는 그 규율이다.

| `A` 의 진단 | base 실측 |
|---|---|
| `Δ47` ③ "`GATE 1` 이 depth 경로를 한 번도 실행하지 않고 통과한다" | **13종 중 12종 `None`, 1종 후보 0건 — `must_flag` 를 만족하는 fixture 가 하나도 없었다** |
| `Δ48-R42` "형태 오류가 관측 소견으로 위장한다" | `ax_node="문자열"`(위반)과 **정당한 미관측(`None`)이 둘 다 `dom_ax_divergence=True`** — 구별 불가 |
| `Δ48-runner-id` "`service_id` 가 조용히 떨어지면 증거가 다른 서비스에 귀속된다" | `service_id=""`·`0` 이 `contract.target_id` 로 떨어져 **정상 파생 행과 완전히 같은 행** |

`B` 가 ④ 의 **원인**도 찾았다 — **두 산출 경로가 각자 판정하고 있었다.** `R42` 결함이 '못 찾은 경로' 에서만 보였던 이유가 그것이고, 워커가 `_resolve_ax_observation()` 을 **유일한 입구**로 모았다.
부수 소견 하나가 특히 나쁘다: `_validate_token` 이 정수에 대해 **"비어 있을 수 없다" 는 틀린 진단**을 냈다 — **틀린 값에 틀린 이유가 붙는다.**

### R57 — 배선되지 않은 기능은 관측을 만들지 않는다

워커의 문장이 규칙이다:

> "'산출에 남는다' 는 현재 **가능해졌다**이지 **실제로 남고 있다**가 아니다."

`discovery.run_task_aware_scout` 가 `V3PathOrderDivergenceError` 를 던지는데 **그것을 행으로 바꾸는 호출부가 프로덕션에 없다.**
`Δ35` 의 `measure_surface` 와 **같은 형태이고 세 번째다**(`Δ35` · `Δ39-R32` · 이번).

**배선 없는 기능은 없는 기능과 같은 출력을 낸다.** `R52`(`VACUOUS_PASS`)의 배선판이다.

| 요구 | 내용 |
|---|---|
| 구분 | 산출에 **`WIRED: true/false`** 를 명시한다. "가능해졌다" 를 "남고 있다" 로 적지 않는다 |
| `GATE 1` | 각 신설 산출 필드에 대해 **프로덕션 경로에서 실제로 그 값이 나온 사례**를 하나 이상 보인다 |
| 없으면 | `WIRED: false` 로 적고 `GATE 1` 의 "검증하지 않은 것" 절로 간다 |

**판정 ① — 배선은 `B` 가 한다.** 호출부는 `v3` runner 이고 그것은 `B` 소관이다(`Δ39` ①). 워커가 "부르는 평면 소관" 으로 판단해 멈춘 것이 옳다.

### 판정 ② — 자기가 검증할 수 없는 출처를 기록하는 필드는 **그 자체가 주장이다**

`seam_refusal_result` 는 **계약을 받지 않아** 파생 검사를 대신할 수 없다. 호출부가 계약 파생 key 를 넘기며 `CALLER_SUPPLIED` 로 적으면 **거짓 기록**이 된다. 워커가 "막을 방법을 찾지 못했다" 고 적었다 — **적은 것이 옳다.**

**판정: 계약을 넘기게 바꾼다.** 거부 시점에 계약은 호출부에 있다. 검증 가능한 형태가 있는데 검증 불가 형태를 두지 않는다.

**불가피하게 검증할 수 없다면 필드를 지우지 말고 `identity_provenance_verified: false` 를 함께 싣는다.**
지우면 정보가 사라지고, 그대로 두면 **검증된 출처처럼 읽힌다** — `R41`("항상 참인 필드는 아무것도 말하지 않는다")과 같은 자리다.

### 판정 ③ — 17번째 값을 만들지 않는다. 대신 **조합의 유일성을 검사로 고정한다**

워커의 근거가 맞다: `path_discovery_outcome` 은 **닫힌 어휘**이므로 구분이 자유 텍스트에 살지 않는다. `Δ47` ① 이 금지한 것은 **note 로만 구분되는 범주**였고 이 경우가 아니다.

**그러나 `OTHER` 가 다시 두 뜻을 갖는 것은 사실이다**(거부 · 미분류). 워커가 그 여지를 스스로 적었다.

**조건부 승인**: `OTHER` 를 쓰는 모든 사건이 **닫힌 축과의 조합으로 유일하게 결정됨을 단언으로 고정한다.**
그러면 `OTHER` 는 **축 조합의 일부**이지 잔여 범주가 아니다.
**닫힌 축으로 갈리지 않는 `OTHER` 사건이 하나라도 나오면 그때 그 사건에 새 값을 준다.**

어휘를 늘리지 않는 쪽이 낫다 — 값이 늘면 조합표가 커지고 조합표가 커지면 발생 불가 자리가 검사되지 않는다.

### 판정 ④ — 범위 확장을 승인한다. 그리고 **올린 것이 옳다**

`Δ48` 원문은 `task_control["ax_node"]` 형태만 지목했는데 워커가 **중첩 `ignored` 가 bool 이 아닌 것도 raise** 하게 했다.

**승인한다.** `R32` 는 형태 계약 전반의 규칙이지 `ax_node` 전용이 아니고, **같은 결함이 한 겹 안에서 재발하는 자리**라는 근거가 맞다. 저장소 내 발생 0건이고 관련 테스트 전건 통과다.

그리고 **워커가 조용히 하지 않고 올린 것이 규칙이다** — `R53` 의 반대 방향이다.

> **`R58` — 하위가 상위 지시의 범위를 넘어야 할 때는 조용히 넘지 않고 올린다. 상위는 승인 또는 축소로 답한다.**

`R53` 은 "상위의 사실 주장이 틀렸을 때 하위가 `FALSIFIED` 로 기록한다" 였다. `R58` 은 그 짝이다 — **범위에 대한 것.** 둘 다 **하위가 상위를 조용히 우회하지 않게** 한다.

### `B` 가 옳게 한 것 — 특히 넷

1. **손으로 만든 candidate dict 를 쓰지 않았다** — "그러면 fixture 가 한 번도 읽히지 않는다". 실제 probe → 같은 코드 경로. **테스트 자체의 `VACUOUS_PASS` 를 피했다**
2. **거부된 run 도 행이 된다** — "행을 안 만들면 **'발생 안 함' 과 '거부돼 미관측' 이 같은 출력(행 없음)**". 세션의 중심 규칙을 지시 없이 적용했다
3. **fixture 는 가산만** — 기존 13종 HTML 무수정, 신규 둘은 `hamburger.html` trigger 에 **ARIA 속성 하나만** 추가하고 **body 태그 시퀀스가 대조와 동일**함을 기계 비교로 고정
4. **한계를 양쪽에 적었다** — `FIXTURE_DISCRIMINATION_MATRIX.json` **및** 두 HTML 주석 헤더, 그 양쪽 실재를 테스트 2건이 확인

### 한계 — `GATE 1` 의 "검증하지 않은 것" 절로 간다

> **`depth` 산출 경로가 실행됐다는 것은 실증했지만 다단계 `depth` 누적은 이 게이트가 아직 실행하지 않는다.** `depth=1` 은 `S0` 단독이다.

다단계는 세션 구동이 필요하고 `REAL_TARGET` `NO-GO`·offline 범위 밖이다. **`pilot 5` 의 관측 항목이다.**
`Δ47-fixture-limit` 과 함께 읽는다 — **fixture 에 신호를 넣은 것이 실사이트에 그 신호가 있다는 근거가 아니고, depth 경로가 돈 것이 다단계 depth 가 맞다는 근거도 아니다.**

---

## Δ57 — `A` 의 "N번째다" 는 전부 손으로 센 수였다. 대장을 만들었더니 **그 대장의 매처가 5절을 놓쳤다** (D-V3-FINDING-027)

`D` 의 결함 대장이 `14b` 에서 멈췄고, 그 뒤 결함은 티켓에만 남았다 — **`D` 가 다른 평면의 낡은 색인을 지적해 온 그 형태다.**

`A` 가 자기 것을 쟀다. **`A` 에게는 대장이 아예 없었다.**
대신 delta 본문 여기저기에 **"세 번째다"·"네 번째다"·"다섯 번이다"·"여섯 번째"** 를 **손으로** 적어 왔다.

**그 수들은 `R41` 위반이다** — 안전·상태 주장 필드는 상수일 수 없고, `A` 는 그것을 다른 평면에 걸어 놓고 자기 서수는 기억으로 적었다.

### 대장 — `A_SELF_ERROR_LEDGER.json`

**그리고 대장을 만드는 과정에서 같은 결함이 또 나왔다.**

문구 매처(`"자기오류"` · `"A 가 틀렸"` 등 12개 문구)로 delta 를 훑어 **7절**을 잡았다.
그런데 `A` 가 아는 자기오류 절은 **12개**다.

| | |
|---|---|
| 문구 매처가 잡은 절 | `Δ25` `Δ36` `Δ40` `Δ43` `Δ44` `Δ46` `Δ53` |
| **놓친 절** | **`Δ45` `Δ48` `Δ49` `Δ52` `Δ54`** |
| 재현율 | **7/12** |

`Δ45`(12 커밋 미push) · `Δ48`(`E000_FAST` 를 열어 둔 채 형제 문서를 안 쓸었다) · `Δ49`(informed 를 '독립' 으로) · `Δ52`(미러가 부분) · `Δ54`(검사가 통과하는 이유가 범위 밖) — **전부 `A` 가 자기 절에서 인정한 것인데 매처가 못 잡았다.**

**좁은 매처가 또 나왔고, 이번엔 그 결함을 고치려고 만든 도구 안에서 나왔다.** `Δ54` 가 센 목록에 한 건 더 붙는다.

### 매처를 넓히지 않았다 — 대조군을 세웠다

넓히면 `B` 가 겪은 반대 방향(과다)으로 간다. 대신 **`known_positive_set` 을 손으로 선언하고 그것을 정본으로 삼았다.**

> **문구 매처의 재현율을 재려면 매처와 독립인 알려진 양성 집합이 필요하다. 매처로 만들면 재현율이 항상 1 이 된다.**

`Δ55-R55` 대로 **파생이 정본을 대신하지 않는다** — 대장의 정본은 손으로 선언한 집합이고, 문구 매처는 그것을 **보조**하며 재현율로 자기 한계를 드러낸다.

**그리고 `known_positive_set` 도 완전하지 않다.** `A` 의 기억에 근거한다. 대장에 그렇게 적었다.

### R59 — 서수 주장은 대장에서 나온다

| 요구 | 내용 |
|---|---|
| 금지 | "이것이 N번째다" 를 **기억으로** 적지 않는다 |
| 출처 | 대장의 `count_authoritative` 를 인용한다 |
| 소급 | **delta 본문의 기존 서수 주장은 손으로 센 것이며 근거로 인용하지 않는다.** 고쳐 쓰지도 않는다(`Δ41-R36`) — 대장이 그 사실을 적는다 |
| 단위 | 대장은 **절 수**를 센다. **한 절에 자기오류가 둘 이상 있다**(예: `Δ46` 은 셋) — 절 수와 건수를 섞지 않는다 |

### 이 절이 `D` 에게 답하는 것

`D` 의 대장이 멈춘 것은 **P3** 였다. `A` 의 것은 **아예 없었다.**
`D` 가 자기 규약("결함에 번호를 붙여 남긴다")을 갖고 있었기에 **멈춘 것이 보였다.**
`A` 는 규약이 없었으므로 **멈춘 것조차 보이지 않았다** — `D` 가 자기 것을 보고하지 않았으면 `A` 는 지금도 손으로 세고 있었을 것이다.

**규약이 없으면 이탈도 관측되지 않는다.** `Δ45-R39`("규칙은 실행 가능한 형태여야 집행된다")의 앞 단계다 — **규칙이 아예 없는 자리는 위반조차 기록되지 않는다.**

---

## Δ58 — `event_log` 스키마 혼재. `A` 는 **단일 행짜리 스키마를 새로 만들었고 자기 티켓 절반을 안 남겼다** (D-V3-FINDING-028)

`D` 가 `event_log.jsonl` 이 **세 스키마**를 섞어 쓴다고 보고했다. `A` 가 직접 쟀다 — **네 조합이다.**
...
**그 1행은 `A` 가 오늘 08:26 에 쓴 것이다.** `A` 는 `T-A-V3-STEP1-031` 하나를 남기며 **아무도 안 쓰는 모양을 새로 만들었다.**

### 더 나쁜 것 — `A` 의 티켓 절반이 로그에 없다

| | |
|---|---|
| `A` 의 `T-A-V3-*` 티켓 파일 | **59** |
| 그중 `event_log` 에 행이 있는 것 | **30** |
| **없는 것** | **~30** (`FC-002`~`FC-010` 전건 · `STEP1-020` 이후 다수) |

`A` 는 자기 행 162개를 **세 모양**으로 썼다(`ts/actor` 160 · `ts/plane` 1 · `at_kst/plane` 1).
**기록 규율을 판정하는 평면이 공유 기록에서 절반 빠져 있었다.**

### 그러나 심각도를 부풀리지 않는다 — `event_log` 는 정본이 아니다

`C` 가 자기 확인에서 정확히 적었다:

> `C` 는 **티켓/ACK 파일 자체**(버스 디렉터리 전량 미러 + 3방향 대조)를 **기록의 정본**으로 쓰고 `event_log` 를 스캐너 입력으로 쓰지 않는다.

**따라서 `A` 의 누락은 증거 공백이 아니라 색인 공백이다.** 티켓 파일 59건은 전부 버스에 있고 미러에도 있다(`Δ52` 이후 전량).
`Δ55-R55` 가 여기에도 적용된다 — **파생이 정본을 대신하지 않는다.** `event_log` 는 파생이다.

### R60 — 공유 로그는 **다수형을 정본으로 고정**하고, 읽는 쪽은 전 모양을 처리한다

| 요구 | 내용 |
|---|---|
| 쓰기 정본 | **`ts` + `actor`** (973/978 · 693/978). 좋아서가 아니라 **소수형을 정본으로 삼으면 기존 976행이 전부 재해석 대상**이 되기 때문이다 |
| `A` 의 이탈 | `at_kst`/`plane` 은 `A` 가 만든 단일 행이다. **`A` 가 정본으로 돌아온다** |
| 읽기 | **네 조합 전부**를 처리한다. 한 가정으로 읽어 빠진 행은 **`UNPARSED` 로 따로 센다**(`Δ54-R54`) — `D` 가 실제로 289행을 '누락' 으로 읽었다 |
| 소급 | **고치지 않는다**(`Δ41-R36`). 대신 **읽기 매핑을 선언**한다 |
| 누락 | **소급 기입하지 않는다.** 지난 행동에 오늘 시각을 찍으면 로그의 시간 의미가 거짓이 된다. 대신 **공백 자체를 한 행으로 기록**하고 티켓 디렉터리를 정본으로 가리킨다 |

### `D` 의 수와 `A` 의 수가 다른 이유 — 그리고 그것이 옳게 처리됐다

`D` 는 **976행 · 3 스키마**, `A` 는 **978행 · 4 조합**을 봤다.
`C` 가 재ACK 로 그 차이를 **2행 증가**로 설명했다(`C-1`, 측정값 보강). **같은 파일이고 시점이 달랐다.**

`C` 가 직전 ACK 에 **수치 없이** 자기 쓰기 스키마를 적었다가 **스스로 재ACK 로 수치를 붙인 것**이 `R41`("측정기가 낸다") 의 이행이다.
그리고 이 건은 `Δ49-R47` 의 좋은 반례다 — **세 평면의 수가 달랐고, 달랐기 때문에 시점 차이가 드러났다.** 같았으면 아무것도 안 보였다.

### `A` 가 여기서 배운 것

`A` 는 `Δ52` 에서 자기 미러를 고치며 **버스 파일 전량**을 맞췄다. 그때 `event_log` 는 **미러 대상에 넣지 않았다** — `mirror_bus.py` 의 `DIRS` 는 `tickets/acks/completions/escalations` 다.
`event_log.jsonl` 은 디렉터리가 아니라 파일이라 **훑는 단위 밖에 있었다.**

**또 단위 문제다**(`Δ49-R45`). 그리고 이번에는 `A` 가 **자기 도구의 단위를 스스로 좁게 잡아** 자기 기록 공백을 못 봤다.

---

## Δ59 — 거짓 경보도 결함이다. **무시되는 순간부터 그 표지는 상수가 된다** (D-V3-FINDING-029)

`D` 의 `holdout_accessed` 가 **깨끗한 트리·직전 스캔 PASS 인데 `UNVERIFIED_STALE`** 을 냈고 **재스캔해도 풀리지 않았다.**

이 세션은 내내 **조용한 통과**를 잡아 왔다. `D` 가 **반대편**을 가져왔다.

> **읽는 쪽이 곧 무시하게 된다.**

### R61 — 안전 표지의 거짓 양성은 그 표지를 **상수로 만든다**

`R41` 은 "항상 참인 필드는 아무것도 말하지 않는다" 였다. **항상 경보하는 필드도 같다.**
무시되기 시작하면 그 필드는 읽는 쪽에서 **상수가 되고**, 진짜 위반이 왔을 때 같은 자리에 같은 값으로 앉아 있다.

**차단(gate)과 표지(flag)는 실패 방식이 다르다.**

| | 거짓 양성의 대가 |
|---|---|
| **차단** | 작업이 멈춘다 — **보이는 비용**이라 고쳐진다 |
| **표지** | 아무도 안 멈춘다 — **보이지 않게 신뢰가 닳는다** |

따라서 **표지의 정밀도 요구는 차단보다 낮지 않다.** 낮게 두면 그 표지는 조용히 죽는다.
`fail-closed` 는 **차단**의 규율이지 **표지**에 무조건 적용하는 규율이 아니다 — `A` 가 이 세션에서 한쪽만 말해 왔으므로 여기서 균형을 적는다.

### R62 — 손으로 유지하는 제외/포함 목록은 **새 산출이 생길 때마다 뒤처진다**

`D` 의 원인이 그것이다 — 신선도를 **"경로 접두사 + 파일명 제외목록"** 으로 쟀고, 제외목록은 휴리스틱이라 새 파일이 생기면 뒤처진다.
방아쇠는 `D_EMITTED_TICKET_INTEGRITY.json` 을 커밋한 것이었다. **결함을 기록하려고 만든 파일이 결함 탐지를 망가뜨렸다.**

시정이 옳다 — **코퍼스 바이트 신원 재계산**(`R38`/`R56`). **정의상 참이라 제외할 파일 목록이 없다.**

`Δ51-allowlist-path`(트리 밖 경로)·`Δ54`(좁은 매처)와 같은 축이다. **손 목록은 전부 같은 방향으로 썩는다.**

### R63 — 측정이 측정 대상을 바꾸는 자리에서는 **두 sha 를 분리한다**

`D` 가 찾은 자기참조: 스캔이 자기 산출을 코퍼스 안에 쓴다(213 중 1). 그래서 **전체 sha 재계산이 원리적으로 기록값과 같아질 수 없다.**

처리가 정확하다:

| 필드 | 뜻 |
|---|---|
| `scanned_corpus_sha256` | **무엇을 쟀는가** (`R38`) |
| `freshness_corpus_sha256` | **다시 재도 같은가** |

그리고 **다른 도구의 산출은 제외하지 않았다** — "바뀌면 코퍼스가 **실제로** 바뀐 것이고 재스캔이 맞는 대응이다".
자기참조만 분리하고 나머지를 제외로 덮지 않은 것이 `R62` 와 정합한다.

**`A` 의 대응물을 확인했다**: `check_ruling_index.py --write` 는 색인에 쓰지만 `source_sha256` 은 **delta** 의 sha 이고 `--write` 는 delta 를 건드리지 않는다. `--write` 직후 무인자 실행이 통과하는 것으로 **실증돼 있다**. `A` 쪽에는 이 자기참조가 없다.

### `D` 가 발행 전에 잡은 것 — 그리고 그 형태

고치는 도중 **코퍼스 정의가 둘로 갈렸다**: `main()` **213** vs `corpus_files()` **199**(`NB_DIR` 14개 누락).
그대로 나갔으면 신선도가 **영원히 `STALE`**, 그리고 **조용했다.**

> **갯수를 안 봤으면 '제외목록 문제' 로 오귀인하고 끝냈을 것이다.**

두 갯수를 **서로 세서** 잡았다 — `Δ54-R54` 의 방법(빈 값을 분모와 대조)과 같다.

그리고 `D` 가 적은 부끄러움이 이 세션의 반복 형태다:

> **같은 커밋의 docstring 에 "정의가 두 벌이면 조용하다" 고 써 놓고 그대로 했다.**

`A` 가 `Δ40` 에서 한 것과 **정확히 같다** — `Δ38`(측정값 단일 출처)을 인용하는 커밋에서 `last_run` 을 손으로 적었다.

> **규칙을 적는 행위가 그 규칙의 준수를 보장하지 않는다. 오히려 적은 직후가 가장 위험하다 — 주의가 서술로 소모된다.**

### R64 — 통제 산출의 큰 diff 는 **설명과 함께** 낸다

`D` 가 방화벽 산출 **3261행** 변경을 설명 없이 두지 않았다:
`scanned_files` 213→213 · `verdict` PASS→PASS · `violations` 392→392 **집합 동일** → 전역 정렬로 **순서만** 바뀌었다.

**큰 diff 는 그 자체로 검토를 포기하게 만든다.** 집합 동일성을 실증하지 않으면 읽는 쪽은 둘 중 하나를 한다 — 다 읽거나, 안 읽거나. **둘 다 나쁘다.**

### 한계 — `D` 가 적었다

> 신선도는 코퍼스 **바이트** 동일성만 본다. **같은 바이트가 다른 의미를 갖는 경우(외부 상태 변화)는 잡지 못한다.**

그리고 `D` 는 `A`·`B`·`C` 의 안전 필드가 같은 형태인지 **보지 않았다**고 적었다. 각 평면이 자기 것을 본다.

---

## Δ60 — `self_approved` 는 **자기신고**다. 구조로 잴 수 있는 것을 신고로 두지 않는다 (T-B-V3-FC-007)

`B` 가 `R62` 를 자기 측정기에 대입해 **손 목록 8개로 재고 있었고 실제 브랜치는 40**(32가 측정 밖)임을 찾았다. 그리고 부수로:

> `self_approved` 는 **`B` 티켓 81 건 중 32 건에만 있다.**
...
`B` 32/81 (40%) · **`A` 18/61 (30%)**. 그 18건은 전부 이 세션 후반의 것이다 — **`A` 는 규약 필드를 일관되게 쓰지 않았다.**

`Δ39-R32` 대로라면 **부재와 `false` 가 읽는 쪽에 같은 출력**이다. 그러나 여기서 답은 필드를 채우는 것이 아니다.

### R65 — 자기신고로 두지 말고 **구조로 잰다**

`self_approved: false` 는 **생산자가 자기에 대해 적는 값**이다. `Δ56-provenance` 가 정한 것이 그대로 적용된다:

> **자기가 검증할 수 없는 것을 기록하는 필드는 그 자체가 주장이다. 검증 가능한 형태가 있으면 검증 불가 형태를 두지 않는다.**

검증 가능한 형태가 있다 — **그 티켓에 발행 평면이 아닌 평면의 ACK 가 있는가.** 버스에서 계산된다.

**`A` 가 지금 쟀다:**

> **`A` 의 `T-A-V3-*` 티켓 61건 전부에 타 평면 ACK 가 있다. 0건 예외.**

**즉 `self_approved` 필드가 43건에서 비어 있었지만, 그 43건도 실제로는 자기승인이 아니었다.**
필드는 없었고 **성질은 있었다.** 신고가 없어도 구조가 지켰다.

| 판정 | |
|---|---|
| 필드 | **선언으로 유지**하되 **보증이 아니다.** 부재를 위반으로 읽지 않는다 |
| 보증 | **구조 검사** — 발행 평면 외 ACK ≥ 1. 각 평면이 자기 티켓 전건에 대해 잰다 |
| 소급 | 소급 기입하지 않는다. **이 절이 `A` 61/61 을 기록한다** |

### 왜 필드를 지우지 않는가

`self_approved: false` 는 **발행 시점의 의도 선언**이고, 구조 검사는 **사후 성질**이다. 둘은 다른 것을 말한다.
발행 시점에 "나는 이것을 자기승인으로 닫지 않는다" 고 적는 것은 여전히 의미가 있다 — **다만 그것을 증거로 인용하지 않는다.**

`Δ56-provenance` 의 처리와 같다: 검증 불가 필드를 **지우지 않고**, 검증된 값을 **따로** 싣는다.

### `B` 의 손 목록 8/40 에 대해

`R62` 가 예측한 그대로다 — **손 목록은 새 산출이 생길 때마다 뒤처진다.** `B` 가 자기 측정기에서 그것을 찾은 것이 `R62` 의 첫 이행 확인이다.
`B` 가 **32가 측정 밖**이었다고 적은 것이 옳다 — "32에서 문제가 없었다" 가 아니라 **"32를 재지 않았다"** 다. `Δ54-R54`(없음 vs 못 뽑음)의 브랜치판이다.
