...
# 개정 1 — 순서 정정 (2026-08-27 16:38, A)

**근거** `claude-b/measurement-recovery@2281c85` · `handoff/RECOVERY_DATAFLOW_AUDIT.md`
(코드 변경 0, 읽기·재집계만. 8필드 × 11단계 보존표, 셀마다 파일:행 근거)

> **이 개정은 데이터를 보고 기준을 바꾼 것이 아니다.** 새 측정값은 없다.
> **코드 구조에 대한 사실**이 확인됐고, 그 사실이 원안의 전제를 무효화했다.
...
```
선행  REC-B-8  가드 배선
 ↓    REC-B-1~3 wiring  (갭 1)
 ↓    REC-B-5  detector (갭 2)
```

**감사 §6.3 판정 — 두 갭은 독립이며 어느 쪽도 다른 쪽을 해소하지 않는다:**
...
```
REC-B-8 (가드)  →  [ REC-B-1~3 wiring  +  REC-B-5 detector ]  를 한 게이트에서 함께 검증
```

> **중간 재수집을 넣지 않는다.** wiring 만 머지하고 재수집하면 결과가 **오늘과 완전히
> 동일**하다. *"고쳤는데 왜 그대로냐"* 가 되고, **그 재수집은 실사이트 접속 예산만 소모한다.**
> 원안은 이 낭비를 지시할 뻔했다.

## §A-2 `REC-B-8` 선행 근거를 정정한다 — 선행은 유지, 이유가 다르다

**원안이 적은 이유:** *"가드가 꺼진 채 wiring 만 고치면 오늘의 '정직한 0' 이 '근거 없는 값' 으로 바뀐다."*

**감사가 좁힌다.** 실사이트에서는 wiring 만으로 승격이 일어나지 않는다(§6.3).
C 가 실증한 승격은 **fixture 환경**에서 성립한 것이다 — fixture 는 `data-endpoint` 를 심는다.

```
fixture 환경   가드 없으면 PENDING 승격 실제로 발생      ← C 실증
실사이트       wiring 만으로는 승격 자체가 불가           ← 감사 §6.3
```

**선행 지위는 유지한다.** 이유는 *"실사이트 오염 방지"* 가 아니라
**"갭2 착수 이후 회귀 스위트가 도는 환경이 곧 fixture 이며, 거기서 가드가 없으면
검증 자체가 오염된다"** 이다. **결론은 같고 근거가 달랐다 — 근거를 고쳐 적는다.**

## §A-3 감사가 새로 등재한 것 (원안에 없던 것)

| | 내용 | 처리 |
|---|---|---|
| **F-1 (P0)** | area/endpoint detector 에 **실사이트용 구현이 아예 없다.** fixture 전용 `data-region`/`data-endpoint` 속성만 읽는다 | `REC-B-5` 를 P0 로 승격. 원안은 이를 P1 수준으로 다뤘다 |
| **F-2 (P1)** | `*_signal_type` 은 **어떤 판정에서도 소비되지 않는다.** 유일한 reader `mapping_frozen_allowed()` 는 **테스트에서만 호출**된다 | `REC-B-8` 의 실체가 "배선 복구" 가 아니라 **"프로덕션 최초 배선"** 이다 |
| **F-3 (P1)** | 목표-수준 가드가 **QUERY 5/5 전건 삭제** — codebook 없이도 area 신호가 성립하는 **유일한** archetype 이었다 | 가드 입도 정밀화의 우선 대상. **단, prohibited action set 은 완화하지 않는다** |
| **F-6** | `*_signal_type` 은 서비스별 값이 아니라 **archetype 의 1:1 함수** — 정보량 0. 다만 detector 설계 범위를 **3종으로 닫아 준다** | 설계 입력으로 채택 |
| **F-7** | C 의 4층위 중 **C-E 회수 가능 규모는 12건이 아니라 1건** | 수치 정정. A 기록 갱신 |
| **§4** | `compute_depth()` 는 **complete-case 가 아니다**(A 가정과 다름). NED 전건 NULL 의 원인은 depth 가 아니라 **area 신호가 한 번도 성립하지 않은 것** | 원인 귀속 정정 |

## §A-4 변경되지 않는 것

```
REAL_TARGET recovery   B 구현 + C 독립검증 + A GO 전까지 금지     유지
prohibited action set  완화하지 않는다 (guard 입도만 정밀화)      유지
ORIGINAL_E001          READ_ONLY                                 유지
오늘의 FINAL           82f631f — recovery 결과가 대체하지 않는다   유지
UNDETERMINED           PASS/endpoint 로 승격하지 않는다            유지
```

> **반사실을 일반화하지 않는다.** 감사는 *"현재 detector 는 fixture 전용 속성만 본다"* 를
> 확인했을 뿐, *"올바른 wiring 과 detector 를 구현해도 depth 는 최대 8"* 을 확인하지 않았다.

---

# 개정 2 — 리플레이 코퍼스 승인 · 게이트 검증력 명시 (2026-08-27 16:45, A)

## §B-1 개정 1 이 남긴 사각 — B 가 발행 5분 만에 찾았다

**개정 1 은 두 갭을 한 게이트로 묶고 중간 재수집을 없앴다. 그래서 픽스처가 유일한 중간 신호가 됐다.**
**그 신호가 갭2 를 검증하지 못한다.**

```
픽스처 마커     [data-region] · [data-endpoint] · body[data-endpoint-reached]
현행 detector   정확히 그 마커를 읽는다 (l1_engine.py:201-231)
```

**픽스처는 detector 가 이미 읽을 줄 아는 것만 공급한다** → `5/5 PASS` 는 **갭1만 증명**한다.

> **위험한 형태:** 픽스처 5/5 PASS 가 나오면 *"두 갭 다 닫혔다"* 로 읽힌다.
> 실제로는 갭1만 닫혔고 갭2 는 R8 까지 미검증으로 남는다.
> **통과 신호가 무엇을 통과시켰는지 말하지 않는다.**

**개정 1 은 `REC-B-5` 를 P0 로 올려놓고 그 게이트의 검증력이 갭1까지라는 것을 보지 않았다.**

## §B-2 승인 — 동결 evidence 오프라인 리플레이를 R2 입력으로 추가

```
코퍼스   artifacts/e001_w0{1..4}/evidence/**/l0a/dom.html   실사이트 DOM 60개
방식     detect_area_signal 오프라인 실행 — 네트워크 접속 0
```
...
| # | 조건 |
|---|---|
| **1** | 금지집합 저촉 없음 — 네트워크 0 · 새 수집 0 · `ORIGINAL_E001 = READ_ONLY` 준수, **복사 없이 원본 경로 참조** |
| **2** | **리플레이 산출은 측정 결과가 아니다** — mart 행 · claim · 등급 부여 대상이 되지 않는다. 그 지위를 산출 파일에 **기계 판독 필드로** 박는다 |
| **3** ★ | **정답을 먼저, detector 를 나중에.** DOM 별 정답을 detector 출력을 보기 전에 라벨링하고, **합격 기준은 DOM 별 정확도**다 — 총 성립 건수·비율이 아니다. 라벨과 구현은 다른 행위자가 한다 |
| **4** | 범위를 산출물에 명시 — 검증 가능: s0 area 검출 / **검증 불가: endpoint 검출**(L1 step DOM 캡처 부재). *"갭2 를 반쯤 연다. 전부는 아니다."* |
| **5** ★ | **게이트가 검증하지 않은 것을 이름으로 말한다** (아래 §B-3) |
| **6** | `assurance/recovery/replay/` 에 배치 — 픽스처(합성 마커)와 **디렉터리 분리**. 섞이면 증거력 차이가 안 보인다 |

> **조건 3 의 이유.** 실DOM 60개에 detector 를 돌려 **성립률을 보면서 고치면
> 원하는 비율이 나올 때까지 조율하게 된다.** outcome-blind 위반이다.
> **"60개 중 N개에서 신호가 잡혔다" 를 성공 지표로 쓰지 않는다 — N 이 크면 좋은 게 아니라
> 정답과 맞아야 좋은 것이다.**

## §B-3 신규 게이트 규칙 — 미검증 항목 병기 의무

**모든 recovery 게이트 판정문은 두 칸을 함께 적는다:**

```
이 게이트가 검증한 것        예) 갭1 wiring · 갭2 area(리플레이 범위)
이 게이트가 검증하지 않은 것  예) 갭2 endpoint — R8 실사이트 전까지 미검증
```

**PASS 옆에 "무엇이 여전히 미검증인가" 가 없으면 PASS 를 발행하지 않는다.**

> 오늘 `E-6b 발화 8 / 구속 1`, `RETRACTED raw 2 / 판정 0` 과 같은 축이다 —
> **통과값 하나만 남기면 그 값이 무엇을 삼켰는지 사라진다.**

## §B-4 변경되지 않는 것

개정 1 §A-4 그대로. **리플레이 승인은 REAL_TARGET GO 가 아니다.**

## §B-5 조건 3 실행 세부 — 라벨러 지정 · 모집단 확정 (16:48)

### 라벨러 — **(b) 전용 워커.** C 라벨링은 채택하지 않는다

```
detector 구현   B
라벨링          전용 워커 — detector 코드를 보지 않은 상태로 DOM 만 보고 라벨
라벨 품질 감사   C
검증 게이트      C
```

**C 라벨링을 채택하지 않는 이유:** C 가 자기 라벨을 기준으로 검증하게 된다.
**"생산자와 판정자가 이름으로 구분되지 않는다"** — 오늘 CLAIM 게이트에서 나온 결함의 재발이다.
**감사자는 라벨을 만들지 않고 라벨을 감사한다.**

### 라벨 동결 — 조건 3 의 실질

```
1  라벨링을 detector 출력을 보기 전에 완료한다
2  라벨 파일을 sha256 으로 동결한다 — detector 착수 전에
3  동결 해시를 게이트 판정문에 기재한다
```

> **동결이 없으면 조건 3 은 선언에 그친다.** 라벨이 언제든 고쳐질 수 있으면
> "정답을 먼저" 는 검증 불가능하다. **해시 동결이 사후 조정을 구조적으로 불가능하게 만드는 유일한 수단이다.**

### 모집단 — **E001 mart 참조분 56 만 쓴다**

| 집합 | n | 처리 |
|---|---|---|
| **E001 mart 참조분** | **56** | **채택** — `fact_landing_observation` 56행과 정확히 일치, 리플레이 결과를 mart 행에 조인 가능 |
| E001 격리분 | 4 | 제외 — mart 밖 |
| E000 | 9 파일 / 6 타깃 (중복 3) | 제외 — 다른 scope · 층화가 다름. **민감도 보조집합으로만 가능하고 주 결과와 합치지 않는다** |

> **`66` 은 두 가지를 센다** — E001 evidence 디렉터리 수(66)와, C 가 계산한 `56+4+6`(단위 혼재)이
> 우연히 같다. **같은 숫자가 다른 것을 셀 때 단위를 붙이지 않으면 일치가 검증처럼 보인다.**
> 모집단 표기에는 **항상 단위(파일/타깃/행)를 붙인다.**
