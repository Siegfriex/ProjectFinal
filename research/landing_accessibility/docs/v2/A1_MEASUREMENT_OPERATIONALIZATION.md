...
area_quantized = floor(area_css_px2 + 0.5)   # 최근접 정수로 반올림
```
...
> 한 말이었으므로 **거짓이며 철회한다.**)
>
> 잔여: 참 면적이 `x.5` 부근인 control 쌍은 반올림 경계를 가로지를 수 있다. 다만 그 위치는
> 정수 면적과 달리 **레이아웃상 특별할 이유가 없어** 체계적으로 재발하지 않는다.
...
| `AREA_QUANTIZATION_UNIT` | `1` (CSS px², 최근접 정수 반올림) | P-C → P-D 검증 | §2.6 규칙 MIN-4. tie-break 2차 키의 양자화 단위 `[V2-C009 시정]` — 이 값이 `selector` 3차 키가 실제로 도달되는지를 정한다. 너무 작으면 서브픽셀 jitter 가 버킷을 가르고, 너무 크면 실제로 다른 크기의 control 이 같은 버킷에 묶인다. §0.5 의 **수집 파라미터**이며 해석 임계값이 아니다 |
