...
- **동일 길이 후보의 면적을 `±0.01px²` · `±0.5px` 선형 흔들림으로 교란했을 때 선택 경로가 유지되는가** (§2.6 MIN-4 양자화) `[V2-C010 시정]` — **부호를 반드시 양쪽 다 태운다.** `+0.01`만 시험하면 정수 명목 면적에서 `−0.01`이 아래 버킷으로 떨어지는 경우를 놓친다(adversarial V2-C009 실측). fixture 는 정수 명목 면적(예: `200×40`)과 비정수 둘 다 포함해야 한다
...
area_quantized = floor(area_css_px2 / AREA_QUANTIZATION_UNIT + 0.5)

AREA_QUANTIZATION_UNIT = 16      # CSS px². P-D 에서 검증·동결 (§7)
```
...
> 한 말이었으므로 **거짓이며 철회한다.**)
>
> **단위는 `1px²`가 아니라 `16px²`다** `[V2-C010 시정]` — 닫는 finding: adversarial V2-C009 #1.
...
>
> `16px²`는 `4×4px` 규모의 실제 레이아웃 차이에 대응한다 — 그보다 작은 차이는
> 동급 control 사이에서 의미를 갖지 않는다. **P-D에서 실측으로 검증·동결한다**(§7).
>
> 잔여: 어떤 단위를 골라도 참 면적이 버킷 경계 부근인 쌍은 남는다. 단위를 키우면 그 확률은
> 줄지만 **실제로 다른 크기의 control이 같은 버킷에 묶이는** 반대 비용이 커진다.
> 이 교환은 `AREA_QUANTIZATION_UNIT`을 동결하는 P-D의 판단 사항이다.
>
> **`area_css_px2`가 `NULL`인 후보** `[V2-C010 시정]` — probe(`l0_probe.js`)는 면적을 못 재면
> `null`을 낸다. 엔진이 `or 0.0`으로 흡수하는 것은 **코드의 우연이지 명세가 아니다**(감사 지적).
> `NULL`은 `0`이 아니다(`01` §11) — 면적 미상 후보는 2차 키에서 **가장 낮은 순위**로 두고
> `selector` 3차 키로 정렬한다. `0`으로 대입해 "가장 작은 control"로 취급하지 않는다.
>
> **양자는 `BRANCHING_LIMIT` 절단선을 움직인다** `[V2-C010 시정]` — 한 버킷 안에서는
> `selector` 문자열이 순서를 정하므로, 같은 버킷의 `z_big`이 잘리고 `a_small`이 들어갈 수 있다.
> MIN-5가 "`BRANCHING_LIMIT`이 최소성의 **범위**를 정한다"고 한 그 범위를
> **양자도 함께 정한다.** 그래서 `AREA_QUANTIZATION_UNIT`은 §7 동결 대상이며,
> 최소성 주장은 "`BRANCHING_LIMIT` · `AREA_QUANTIZATION_UNIT` · 후보 지명 규칙 ·
> 즉시종료 규칙 아래에서 관측된 최소"로 읽어야 한다.
...
"`BRANCHING_LIMIT` · `AREA_QUANTIZATION_UNIT` · 후보 지명 규칙 · 즉시종료 규칙 아래에서 관측된 최소 activation 수"다 `[V2-C010 시정]`.
...
| `AREA_QUANTIZATION_UNIT` | `16` (CSS px², 최근접 배수로 반올림) `[V2-C010 시정]` | P-C → P-D 검증 | §2.6 규칙 MIN-4. tie-break 2차 키의 양자화 단위 `[V2-C009 시정]` — 이 값이 `selector` 3차 키가 실제로 도달되는지를 정한다. 너무 작으면 서브픽셀 jitter 가 버킷을 가르고, 너무 크면 실제로 다른 크기의 control 이 같은 버킷에 묶인다. §0.5 의 **수집 파라미터**이며 해석 임계값이 아니다 |
