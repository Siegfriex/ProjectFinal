...
def render_limitations(mart_dir: Path) -> str:
    ...
    add("")
    add(
        "이 조건은 **반사실 결론 하나**에 붙는다. 원인 귀속표 **전체**에 붙는 더 넓은 "
        "조건은 §11을 보라."
    )
    add("")
    ...
    # ── 11 — 원인 귀속표 **전체**에 붙는 조건 (A 확정 15:32) ──
    add("## 11. 원인 귀속표 전체의 적용 범위 — recovery lane 감사 중")
    add("")
    add(
        "depth 축 미산출의 원인 귀속(가드 입도 · gate 판별 · archetype-endpoint 규칙)은 "
        "**현재 구현 하에서의 분해**다. task definition이 측정기에 전달되는지 여부는 "
        "post-E001 recovery lane에서 **별도 감사 중**이며, **그 결과에 따라 원인 구조가 "
        "달라질 수 있다.**"
    )
    add("")
    add(
        "**오늘의 원인 분해가 틀렸다는 뜻이 아니다.** 그것은 **관측 가능한 층위에서의 "
        "분해**이며, 그 아래 층이 지금 감사 중이라는 뜻이다. task definition이 "
        "`CODEBOOK_PENDING`으로 고정돼 Scout에 도달하지 못한다는 관찰이 사실로 확인되면, "
        "가드 25건·archetype 규칙 11건 같은 분해는 **'그 위에 더 근본적인 원인이 "
        "있었다'로 재해석될 수 있다.**"
    )
    add("")
    add(
        "이 조건을 지금 적어두지 않으면, 나중에 recovery 결과가 나왔을 때 **오늘 "
        "산출물이 원인을 확정한 것처럼 읽힌다.**"
    )
    add("")
    add(
        "§4의 조건과 성격은 같지만 **범위가 다르다** — §4는 반사실 결론(회복 상한 "
        f"{recovery['depth_recovery_upper_bound']}) 하나에 붙는 조건이고, 이 항목은 "
        "**원인 귀속표 전체**에 붙는다."
    )
    add("")
    return "\n".join(lines)
