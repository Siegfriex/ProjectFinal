...
bad_page_hash: list[tuple[Any, str | None, str | None]] = []
declared_cards = 0
measured_cards = 0
page_card_measured = {}
...
for p in rsm["page_hashes"]:
    ...
    for idx, art in enumerate(ART.findall(html)):
        ...
        h3_m = H3.search(art)
        org_m = ORG.search(art)
        sta_m = STA.search(art)
        reparsed.append(
            {
                "certification_number": d.group(2) if d else None,
                "service_name": untag(h3_m.group(1)) if h3_m else None,
                "organization_name": untag(org_m.group(1)) if org_m else None,
                "status_raw": untag(sta_m.group(1)) if sta_m else None,
                "cert_start_date": st,
                "cert_end_date": en,
...
bad_seq: list[tuple[Any, Any, list[Any]]] = []
...
emit()
mc_bad: list[tuple[Any, int | str, Any]] = []
...
pm_bad: list[tuple[Any, list[str], list[str]]] = []
for _, r in panel.iterrows():
    v = r["metric_columns"]
    try:
        parsed = json.loads(v) if isinstance(v, str) else list(v)
        names = {str(m.get("name")) if isinstance(m, dict) else str(m) for m in parsed}
    except Exception:
        continue
    got_names = {str(x) for x in rows.loc[rows["panel_id"] == r["panel_id"], "metric_name"]}
    if names != got_names:
        pm_bad.append((r["panel_id"], sorted(names), sorted(got_names)))
emit(f"T13 panel.metric_columns names != distinct metric_name in rows: {len(pm_bad)}")
for pm_row in pm_bad[:10]:
    emit(f"    {pm_row}")
if pm_bad:
    finding(
