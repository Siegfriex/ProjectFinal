...
이 모듈은 `A2_VOCABULARY_AND_SCHEMA_BINDING.md`가 이 워크트리(base `2025e56`,
`agent/landing-pc-fixture` cascade는 아직 merge되지 않았다)에 없으므로, A2 §1이
확정한 닫힌 집합(closed vocabulary) 중 이 7개 표에 실제로 쓰이는 컬럼만 **로컬로
다시 선언**한다. 새 값을 만들지 않는다 — 전부 A2 원문에서 그대로 옮겼다(각주 참조).
...
#: 01_DATA_SPEC §9 `fact_ai_adjudication`. `review_item_id`가 grain이며, 이는
#: P-C의 AI review cascade(`ai_review.py` `AdjudicationRecord`, 이 워크트리에는
#: 아직 없다)가 산출하는 행과 **같은 컬럼 계약**을 공유하도록 맞췄다 — cascade의
#: 출력을 받는 것이 이 lane의 역할이지 cascade 자체를 다시 만드는 것이 아니다.
...
#: 01_DATA_SPEC §8 `dim_certification`.
