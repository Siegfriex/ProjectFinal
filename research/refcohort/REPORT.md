...
**작성 시각** 2026-08-26 (KST) · **브랜치** `research/refcohort-r1` · **최신 커밋** `2.1.3 기준 시정 반영`
...
| — | SSOT 해설서 원문 대조로 2.1.3 임계값 오류 발견 | 아래 3-3 |
| **R4** | 2.1.3을 대각선 6mm 기준으로 시정 후 **재판정** | 257 판정 / guard OK |

R3·R4는 사이트에 다시 요청하지 않는다. `rejudge.py`가 보존된 probe 증거로 판정만 다시 한다 — 부하가 없고 원 증거 해시가 유지된다.
...
|---|---:|---:|---:|---:|---:|---:|
| 인증 보유군 | 226 | 216 | 10 | **12** | 3.54 (0~8) | 2,679 / 91,479 |
| 실사용 상위군 | 48 | 41 | 7 | **0** | 5.90 (3~8) | 2,527 / 25,574 |
...
| 2.1.3 | 조작 가능 | 판정 | 256 | 105 | 41.0% | 96.1% |
| 3.3.2 | 레이블 제공 | 판정 | 108 | 43 | 39.8% | 66.4% |
| 2.4.2 | 제목 제공 | 판정 | 257 | 85 | 33.1% | 86.9% |
| 3.2.1 | 사용자 요구에 따른 실행 | 신호 | 20 | 5 | 25.0% | 82.9% |
| 4.1.1 | 마크업 오류 방지 | 판정 | 257 | 56 | 21.8% | 77.7% |
| 1.3.2 | 콘텐츠의 선형구조 | 신호 | 257 | 46 | 17.9% | 82.1% |
| 2.4.1 | 반복 영역 건너뛰기 | 판정 | 257 | 43 | 16.7% | 83.3% |
| 1.1.1 | 적절한 대체 텍스트 | 신호 | 257 | 40 | 15.6% | 99.2% |
| 1.3.1 | 표의 구성 | 판정 | 26 | 4 | 15.4% | 85.4% |
| 2.1.1 | 키보드 사용 보장 | 신호 | 257 | 34 | 13.2% | 99.0% |
| 2.1.2 | 초점 이동 | 신호 | 257 | 10 | 3.9% | 99.7% |
| 3.1.1 | 기본 언어 표시 | 판정 | 257 | 10 | 3.9% | 96.1% |
| 3.2.2 | 찾기 쉬운 도움 정보 | 신호 | 189 | 0 | 0.0% | 100.0% |

표본이 작은 항목(1.2.1 자막 3건 100%, 3.3.4 반복입력 6건 100%, 3.3.3 인증 12건 58.3%)은
**분모가 작아 기사에 비율로 쓰면 안 된다.**

### 3-3. 2.1.3 조작 가능 — 기준 시정 이력

R3까지 이 항목은 미흡률 89.8%로 1위였다. 임계값이 틀려서였다.

KWCAG 2.2 해설서 2.1.3 체크사항 원문:

> CSS의 픽셀 크기를 기준으로 **대각선 길이 6mm 이상**으로 구현해야 합니다.
> CSS의 픽셀은 1인치/96으로 약 가로 x 세로 0.26mm이 되며 **가로 세로 약 17px이 대각선 길이가 6mm**가 됩니다.

기준은 **최소변이 아니라 대각선**이고, 단위는 CSS 96dpi 환산 **mm**다.
초기 구현은 WCAG 2.5.8을 차용해 `최소변 ≥ 24 CSS px`를 썼는데, 이러면
19×19px 버튼(대각선 26.9px = 7.11mm, 해설서 기준 통과)이 FAIL로 찍힌다.

| 라운드 | 기준 | 적용 | 미흡 | 미흡률 | 지점 통과율 |
|---|---|---:|---:|---:|---:|
| R1~R3 | 최소변 ≥ 24 CSS px (WCAG 2.5.8 차용) | 256 | 230 | 89.8% | 75.9% |
| **R4** | **대각선 ≥ 6mm (KWCAG 해설서)** | 256 | **105** | **41.0%** | **96.1%** |

경계 검증: 17×17 → 6.36mm PASS / 16×16 → 5.99mm FAIL — 해설서의 "약 17px" 서술과 일치한다.
인접 간격 구제 규칙도 제거했다. 해설서가 간격에 대한 수치 기준을 제시하지 않기 때문이다
(간격은 참고 지표로 계속 기록한다).
...
# 코호트 요약
python -c "
import json; r=json.load(open('runs/r4/report.json'))
...
python -c "
import json
rows=[json.loads(l) for l in open('runs/r4/records.jsonl')]
...
python rejudge.py r2 <run_id>         # 재요청 없이 기존 증거로 판정만 다시 (R3/R4가 이 경로)
```
...
```bash
cat runs/r4/guard_report.json   # status: OK / WARN / HALT
```
...
| `runs/r4/report.json` | **최종 집계** |
| `runs/r4/records.jsonl` | 서비스별 전체 판정 (4.6MB) |
...
3표 전원 확인을 통과한 발견 18건(CRITICAL 5 / HIGH 9 / MEDIUM 4) 중 **2.2.2 관련 1건과 2.1.3 임계값(`target-size-overstrict-gap-rule`) 1건, 총 2건만 반영**했다. 나머지 16건은 미시정이다.
...
- ~~2.1.3 임계값 재확인 필요~~ → **R4에서 해설서 원문 기준(대각선 6mm)으로 시정 완료.** 미흡률 89.8%→41.0%.
...
2. ~~2.1.3 임계값 재확인~~ — **완료(R4)**. 다음은 1.4.3 명도대비(74.9%)와 1.4.2 자동재생(83.3%)을 같은 방식으로 해설서 원문 대조.
