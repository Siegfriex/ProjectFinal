...
def test_eda07_allows_comparison_when_variance_present(
    tmp_path: Path, marts: dict[str, pd.DataFrame]
) -> None:
    from analysis.eda.eda07_certification_descriptive import run_eda07
    ...
    summary = json.loads(paths.summary_json_path.read_text())
    assert summary["mode"] == "COMPARISON_ELIGIBLE"
    assert "comparison_decision_coverage_by_certified_current" in summary
